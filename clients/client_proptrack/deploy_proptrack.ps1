# deploy_proptrack.ps1 - one-shot, idempotent stand-up of the entire client_proptrack pipeline on GCP.
#
...
#
#   HOW TO RUN (from the repo root OR from inside client_proptrack\):
...
#
#   Like client_STT, PropTrack's job is read-only on BigQuery and reads the shared raw layers
#   directly (no Snowflake connection, no src_* landing). So: create dataset -> apply views ->
#   build/deploy job -> run -> dash. Single currency AUD; there is no FX step anywhere.

# ---- config (matches job/cloudbuild.yaml + dash/cloudbuild.yaml) ------------
$PROJECT  = "bidbrain-analytics"
$REGION   = "australia-southeast1"
$REPO     = "bidbrain"                                 # Artifact Registry docker repo (shared)
$DATASET  = "client_proptrack"
$BUCKET   = "bidbrain-analytics-proptrack-dash"
$JOB      = "proptrack-export"
$SERVICE  = "proptrack-dash"
$JOB_SA   = "proptrack-dash-job@${PROJECT}.iam.gserviceaccount.com"
$WEB_SA   = "proptrack-dash-web@${PROJECT}.iam.gserviceaccount.com"
$PW_SECRET      = "proptrack-dash-password"
$SESSION_SECRET = "proptrack-dash-session-key"
...
# Build context for `gcloud builds submit .` is the repo root. If we're inside client_proptrack\, step up.
if (-not (Test-Path 'clients/client_proptrack/job/cloudbuild.yaml')) {
  if ((Test-Path 'job/cloudbuild.yaml') -and ((Split-Path -Leaf (Get-Location)) -eq 'client_proptrack')) {
    Set-Location ../..; Write-Host "Moved up to repo root: $(Get-Location)"
  } else { Write-Error "Run from the repo root or from inside client_proptrack\."; exit 1 }
}

Write-Host "Deploying client_proptrack to $PROJECT ($REGION)`n"
...
Ensure-Sa $JOB_SA "PropTrack dashboard export job"
Ensure-Sa $WEB_SA "PropTrack dashboard web service"
...
$sqlFiles = Get-ChildItem 'clients/client_proptrack/sql' -Filter '*.sql' | Sort-Object Name
...
gcloud builds submit clients/client_proptrack/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
gcloud run jobs execute $JOB --region $REGION --project $PROJECT --wait; Must "run export job (write proptrack.json)"
...
if (Test-Path 'clients/client_proptrack/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit clients/client_proptrack/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
  gcloud run deploy $SERVICE --image $WEB_IMG --region $REGION --service-account $WEB_SA `
    --set-env-vars "GCS_BUCKET=${BUCKET},DATA_OBJECT=proptrack.json" `
  ...
  Write-Host "  SKIPPED dash service - client_proptrack\dash\dashboard.html not found."
}
