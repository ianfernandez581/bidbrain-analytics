# deploy_burnet.ps1 - one-shot, idempotent stand-up of the client_burnet PREVIEW dashboard.
#
# PREVIEW ONLY. No Burnet feed is connected, so this client has NO data pipeline: no BigQuery
...
#
# This mirrors the Geyer Valmont / Lacevo preview clients. Like those there is deliberately no
# -WithData switch, because there is nothing to switch on yet - see "FLIPPING PREVIEW TO LIVE" in
# README.md for the full ordered checklist.
#
#   HOW TO RUN (from the repo root OR from inside client_burnet\):
#       .\clients\client_burnet\deploy_burnet.ps1
...
$REPO     = "bidbrain"
$BUCKET   = "bidbrain-analytics-burnet-dash"
$SERVICE  = "burnet-dash"
$WEB_SA   = "burnet-dash-web@${PROJECT}.iam.gserviceaccount.com"
$PW_SECRET      = "burnet-dash-password"
$SESSION_SECRET = "burnet-dash-session-key"
...
# fails with
#   HTTPError 400: Service account burnet-dash-web@... does not exist
...
# Build context is the repo root. If we're inside client_burnet\, step up.
if (-not (Test-Path 'clients/client_burnet/dash/Dockerfile')) {
  if ((Test-Path 'dash/Dockerfile') -and ((Split-Path -Leaf (Get-Location)) -eq 'client_burnet')) {
    Set-Location ../..; Write-Host "Moved up to repo root: $(Get-Location)"
  } else { Write-Error "Run from the repo root or from inside client_burnet\."; exit 1 }
}

Write-Host "Deploying client_burnet PREVIEW to $PROJECT ($REGION)`n"
...
if (-not (Exists { gcloud iam service-accounts describe $WEB_SA --project $PROJECT })) {
  gcloud iam service-accounts create ($WEB_SA.Split('@')[0]) --display-name "Burnet dashboard web service" --project $PROJECT; Must "create web SA"
}
...
# dashboard at /d/burnet/ and logs into the upstream ON THE USER'S BEHALF (main.py _upstream_pw ->
...
# happened on the Geyer Valmont standup. secretVersionAdder + serviceAccountUser are the
...
gcloud builds submit clients/client_burnet/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
gcloud run deploy $SERVICE --image $WEB_IMG --region $REGION --service-account $WEB_SA `
  --set-env-vars "GCS_BUCKET=${BUCKET},DATA_OBJECT=burnet.json,CLIENT_KEY=burnet" `
...
Write-Host "`nNOTE: run scripts\enable_platform_sso.ps1 -Keys burnet to add burnet to the SSO allow-list"
...
Write-Host "`n============================================================"
Write-Host "  DONE. Burnet PREVIEW dashboard is live (password-gated):"
Write-Host "    $URL"
Write-Host "  Showing SAMPLE data behind the preview notice (bucket is empty)."
Write-Host "  Portal tile: run bidbrain-platform\dash\set_burnet_tile.py --yes"
Write-Host "  To go live later: see FLIPPING PREVIEW TO LIVE in clients\client_burnet\README.md"
Write-Host "============================================================"
