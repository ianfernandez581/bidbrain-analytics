# deploy_dash_schneidersecpwr.ps1 - redeploy ONLY the schneidersecpwr dashboard SERVICE after editing
...
# For data-shape changes use deploy_job_schneidersecpwr.ps1; for SQL view changes use
# deploy_views_schneidersecpwr.ps1.
# For first-time standup (APIs, SAs, IAM, secrets, scheduler) use the one-shot deploy_schneidersecpwr.ps1.
#
#   HOW TO RUN (from anywhere - paths resolve from the script's own folder):
#       .\client_schneidersecpwr\dash\deploy_dash_schneidersecpwr.ps1
#   If you get "running scripts is disabled on this system":
#       Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force

# ---- config (matches deploy_schneidersecpwr.ps1 + dash/cloudbuild.yaml) -----------
