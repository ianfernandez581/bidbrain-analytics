"""Schneider Electric — Secure Power dashboard web app (Cloud Run service).

Thin password gate + static server. It renders a login screen, and once a session is
authenticated it serves `dashboard.html` and proxies the private `schneidersecpwr.json` from
...
<title>Schneider Electric · Secure Power</title>
<style>
...
def report_route():
    ...
    ckey = "".join(c for c in str(summary.get("client") or "schneidersecpwr").lower()
