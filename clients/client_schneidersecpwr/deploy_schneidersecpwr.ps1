# deploy_schneidersecpwr.ps1 - one-shot, idempotent stand-up of the entire client_schneidersecpwr pipeline on GCP.
#
...
#
#   HOW TO RUN (from the repo root OR from inside client_schneidersecpwr\):
#       .\client_schneidersecpwr\deploy_schneidersecpwr.ps1
...
#   create dataset -> apply views -> build/deploy job -> run -> dash.
...
$BUCKET   = "bidbrain-analytics-schneidersecpwr-dash"
$JOB      = "schneidersecpwr-export"
$SERVICE  = "schneidersecpwr-dash"
$JOB_SA   = "schneidersecpwr-dash-job@${PROJECT}.iam.gserviceaccount.com"
$WEB_SA   = "schneidersecpwr-dash-web@${PROJECT}.iam.gserviceaccount.com"
$PW_SECRET      = "schneidersecpwr-dash-password"
$SESSION_SECRET = "schneidersecpwr-dash-session-key"
...
# Build context for `gcloud builds submit .` is the repo root. If we're inside client_schneidersecpwr\, step up.
if (-not (Test-Path 'clients/client_schneidersecpwr/job/cloudbuild.yaml')) {
  if ((Test-Path 'job/cloudbuild.yaml') -and ((Split-Path -Leaf (Get-Location)) -eq 'client_schneidersecpwr')) {
    Set-Location ../..; Write-Host "Moved up to repo root: $(Get-Location)"
  } else { Write-Error "Run from the repo root or from inside client_schneidersecpwr\."; exit 1 }
}

Write-Host "Deploying client_schneidersecpwr to $PROJECT ($REGION)`n"
...
Ensure-Sa $JOB_SA "Schneider dashboard export job"
Ensure-Sa $WEB_SA "Schneider dashboard web service"

# JOB SA: run BigQuery jobs + read-only across BigQuery (the views read raw_snowflake; the job
# writes nothing to BigQuery), and write the data bucket. Dataset-scoped grants need an org
...
& $PYTHON 'clients/client_schneidersecpwr/create_views.py'; Must "apply views"
$JOB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${JOB}:${SHA}"
gcloud builds submit clients/client_schneidersecpwr/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
gcloud run jobs execute $JOB --region $REGION --project $PROJECT --wait; Must "run export job (write schneidersecpwr.json)"
...
if (Test-Path 'clients/client_schneidersecpwr/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit clients/client_schneidersecpwr/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
  gcloud run deploy $SERVICE --image $WEB_IMG --region $REGION --service-account $WEB_SA `
    --set-env-vars "GCS_BUCKET=${BUCKET},DATA_OBJECT=schneidersecpwr.json" `
  ...
  Write-Host "  SKIPPED dash service - client_schneidersecpwr\dash\dashboard.html not found."
}
