...
dependency order: the stg_* filter views — 01/02/03/03b/03c — before the 04–23
rollups that read them). The export job (client_STT/job/main.py) SELECTs from these
views to assemble stt.json.

DIVERGENCE FROM client_mongodb / client_cloudflare:
STT reads its four sources straight from the shared raw layers, all in Snowflake
(raw_snowflake.{google_analytics_apac_all, linkedin_ads_apac, dv360_apac,
google_ads_apac}), so there is NO src_* landing step and NO bootstrap-first-failure.
