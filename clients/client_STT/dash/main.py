"""STT GDC APAC dashboard web app (Cloud Run service).
...
`stt.json` from GCS at `/data.json`. All presentation logic — the Overview /
Paid Media / Website Traffic / Ads → Traffic tabs and every chart — lives in
`dashboard.html`; this file only decides *who* may see it, not *what* it shows.

Same service pattern as client_mongodb/dash/main.py (byte-for-byte on the
...
DATA_OBJECT = os.environ.get("DATA_OBJECT", "stt.json")      # object inside it
...
<title>STT GDC · APAC Dashboard</title>
<style>
  *{box-sizing:border-box}
  ...
  .brand{font-size:11px;font-weight:700;letter-spacing:1.6px;color:#D62828;margin-bottom:14px;text-align:center}
  h1{font-size:18px;font-weight:700;margin:0 0 4px;color:#23272E;text-align:center}
  p{font-size:13px;color:#6B7480;margin:0 0 22px;text-align:center}
  input{width:100%;padding:12px 13px;font-size:15px;color:#23272E;background:#fff;
        border:1px solid #E6E9ED;border-radius:10px;outline:none}
  input:focus{border-color:#D62828;box-shadow:0 0 0 3px rgba(214,40,40,.12)}
  button{width:100%;margin-top:12px;padding:12px;font-size:15px;font-weight:700;cursor:pointer;
         background:#D62828;color:#fff;border:none;border-radius:10px}
  button:hover{background:#B81F1F}
...
/* BB-LOGIN-KIT:css v1 */
  ...
  /* ==========================================================================
     ...
     ========================================================================== */
  :root{--bl-accent:rgb(214,40,40);--bl-glow:rgba(214,40,40,0.42);--bl-ease:cubic-bezier(.22,1,.36,1)}
  ...
  .bb-lgfx .o1{width:62vw;height:56vh;top:-16%;left:-10%;animation:blOrb1 24s ease-in-out infinite;
    background:radial-gradient(circle,rgba(214,40,40,0.2) 0%,transparent 68%)}
  .bb-lgfx .o2{width:54vw;height:48vh;bottom:-18%;right:-12%;animation:blOrb2 29s ease-in-out infinite;
    background:radial-gradient(circle,rgba(14,124,134,0.14) 0%,transparent 68%)}
  .bb-lgfx .o3{width:46vw;height:42vh;top:28%;right:4%;animation:blOrb3 33s ease-in-out infinite;
    background:radial-gradient(circle,rgba(185,116,0,0.11) 0%,transparent 70%)}
  ...
  .bb-pw-t:hover{color:var(--bl-accent);border-color:var(--bl-accent);background:rgba(214,40,40,0.1)}
