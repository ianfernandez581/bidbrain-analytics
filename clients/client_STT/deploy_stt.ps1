# deploy_stt.ps1 - one-shot, idempotent stand-up of the entire client_STT pipeline on GCP.
#
...
#
#   HOW TO RUN (from the repo root OR from inside client_STT\):
#       .\client_STT\deploy_stt.ps1
...
#
#   DIVERGENCE FROM clients/client_cloudflare/deploy_cloudflare.ps1: STT's job is read-only on BigQuery and
...
$DATASET  = "client_stt"
$BUCKET   = "bidbrain-analytics-stt-dash"
$JOB      = "stt-export"
$SERVICE  = "stt-dash"
$JOB_SA   = "stt-dash-job@${PROJECT}.iam.gserviceaccount.com"
...
# Build context for `gcloud builds submit .` is the repo root. If we're inside client_STT\, step up.
if (-not (Test-Path 'clients/client_STT/job/cloudbuild.yaml')) {
  if ((Test-Path 'job/cloudbuild.yaml') -and ((Split-Path -Leaf (Get-Location)) -eq 'client_STT')) {
    Set-Location ../..; Write-Host "Moved up to repo root: $(Get-Location)"
  } else { Write-Error "Run from the repo root or from inside client_STT\."; exit 1 }
}

Write-Host "Deploying client_STT to $PROJECT ($REGION)`n"
...
$sqlFiles = Get-ChildItem 'clients/client_STT/sql' -Filter '*.sql' | Sort-Object Name
...
gcloud builds submit clients/client_STT/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
gcloud run jobs execute $JOB --region $REGION --project $PROJECT --wait; Must "run export job (write stt.json)"
...
if (Test-Path 'clients/client_STT/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit clients/client_STT/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
  gcloud run deploy $SERVICE --image $WEB_IMG --region $REGION --service-account $WEB_SA `
    --set-env-vars "GCS_BUCKET=${BUCKET},DATA_OBJECT=stt.json" `
  ...
  Write-Host "  SKIPPED dash service - client_STT\dash\dashboard.html not found."
}
