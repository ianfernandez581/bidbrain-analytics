...
#
#   DIVERGENCE FROM clients/client_cloudflare/deploy_cloudflare.ps1: HireRight's job is read-only on BigQuery and
...
if (-not (Test-Path 'clients/client_hireright/job/cloudbuild.yaml')) {
...
gcloud builds submit clients/client_hireright/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
if (Test-Path 'clients/client_hireright/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit clients/client_hireright/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
