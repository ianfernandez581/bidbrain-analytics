-- 06_by_campaign: per-campaign metric row (the Optimise table). One row per campaign.
CREATE OR REPLACE VIEW `bidbrain-analytics.client_geocon.by_campaign` AS
SELECT
  campaign_name,
  funnel_stage,
  currency,
...
GROUP BY campaign_name, funnel_stage, currency