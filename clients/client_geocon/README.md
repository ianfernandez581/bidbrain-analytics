...
### Enquiry reporting (withheld 2026-09-03, restored 2026-09-17)

**State today: REPORTED (`LEADS_REPORTABLE = true`), restored 2026-09-17 on the agency's
instruction.** The precondition the original client instruction named has NOT been met - **Salesforce
is not connected**, verified 2026-09-17 three ways: this client's `sql/` reads Meta / LinkedIn /
Trade Desk / Google Ads / GA4 and no CRM source; the estate's only Salesforce mirror
(`raw_snowflake.salesforce_cs_apac_all`, Transmission's Snowflake) returns **zero** Geocon rows; and
Windsor has no Salesforce connector configured for any client. So the enquiry figure on screen is a
Meta lead-form submit count that has never been near a CRM, so the `renderNote()` explainer is now
the only thing between the reader and a figure they could mistake for pipeline. The modelled
qualified-lead tile that used to sit beside it was deleted the same day - see below.

**The original instruction, kept because it is why the machinery exists:**
