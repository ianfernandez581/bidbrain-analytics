...
| **LinkedIn** | **STILL BLOCKED - the connector grant.** Account **556629043** ("Geocon Group", Active) exists with 3 campaigns / 5 ad sets / 16 ads, all PAUSED - waiting on launch. **Client-confirmed 2026-09-01: this delivery is NORTHBOURNE GATEWAY's**, even though the one visible campaign is named "Gateway Braddon Aug2026" (id 1194493866). That name carries no Northbourne token, so it would land in `Unmapped` and be excluded from every KPI - `sql/07_stg_linkedin.sql` carries the exact one-line account-scoped fallback to apply at go-live, recorded as an instruction rather than an inference. Original note follows. **STILL BLOCKED - the only outstanding grant.** The connector carries APJC / STT / Cloudflare / Schneider / PropTrack / HireRight / ResetData and nothing else; there is no Geocon account on it. `sql/07_stg_linkedin.sql` is written and returns zero rows. A$6,000 (seq 4). |
