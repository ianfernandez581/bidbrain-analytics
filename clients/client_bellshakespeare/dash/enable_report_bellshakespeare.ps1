# enable_report_bellshakespeare.ps1 - ONE-TIME standup for the "Download report" feature (dash/report.py).
...
# bellshakespeare-dash service, and bumps the Cloud Run request timeout so the two-stage Claude/Gemini call
...
#       .\clients\client_bellshakespeare\dash\enable_report_bellshakespeare.ps1 -Key "sk-ant-..." -GeminiKey "AQ...."
#   If you get "running scripts is disabled on this system":
#       Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force
#
# After this runs once, redeploy the dashboard image normally with deploy_dash_bellshakespeare.ps1 - the
...
$SA_FALLBACK = "bellshakespeare-dash-web@bidbrain-analytics.iam.gserviceaccount.com"   # dash runtime SA (convention)
$SECRET   = "anthropic-api-key"
$GSECRET  = "gemini-api-key"
$BUCKET   = "bidbrain-analytics-bellshakespeare-dash"   # private data bucket - report cache lives in reports/
...
else { Write-Host "bellshakespeare-dash runtime SA: $SA" }
...
Write-Host "Redeploy the dashboard image with deploy_dash_bellshakespeare.ps1 to ship report.py + the new dashboard.html."
