...
Bell Shakespeare — Bell Shakespeare Meta paid-media campaign. Turns this client's LIVE numbers into a
...
- Slide 2 "Why did it happen?" — why numbers are up/down/flat, mixing Bell Shakespeare's own numbers with cited external context: a summary plus ranked drivers (title, explanation, evidence tying to a number, direction, confidence).
...
- Keep client metrics and external benchmarks clearly distinct in wording (e.g. "CTR 0.71% vs benchmark ~0.9%"). Never let a web/context figure masquerade as one of Bell Shakespeare's own performance numbers.
...
- Audience: the Bell Shakespeare marketing lead / executive sponsor who is NOT a media specialist and has ~60 seconds. Optimise for instant clarity and persuasion.
...
def _fmt_brief(s):
    ...
    Reads Bell Shakespeare's payload shape (context / overview / targets / by_stage / by_campaign / top_ads /
    ...
    L.append("")
    L.append(f"Bell Shakespeare campaign report — {s.get('generated_for','Bell Shakespeare — Bell Shakespeare')}. Currency: {cur}.")
    ...
    L.append("")
    L.append("## CAMPAIGN")
    L.append(f"Client: {s.get('client','bellshakespeare')}  |  Development: {ctx.get('campaign','Bell Shakespeare')}  |  Currency: {cur}")
