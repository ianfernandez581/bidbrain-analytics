# client_bellshakespeare — Bell Shakespeare (100% Digital) — PLACEHOLDER / ONBOARDING

> **Status (2026-07-04): placeholder.** This is a full, Bell Shakespeare-branded dashboard scaffold **cloned
> from `client_geocon`** (the lean Meta paid-media template) for onboarding **Bell Shakespeare** as a **100%
...
>
> **Theme:** a **LIGHT** treatment matching Bell's brand — warm off-white canvas (`#F4F3EE`), near-black
> ink, and a **sage-green accent** (`#3E6B4F`) — vs the dark house style, because Bell Shakespeare's
> identity is black/white/sage (the same recolour-the-`:root` approach, just inverted). Logo = Bell's
> flipped **"BELL / SHAKESPEARE."** wordmark recreated in CSS (swap in the official SVG when available);
> the 100% Digital light logo sits in the topbar. Regenerate the sample data with
> `.\.venv\Scripts\python.exe clients\client_bellshakespeare\gen_placeholder.py`.
>
> **TBD at onboarding (assumptions inherited from the template — revisit with the client):**
> the real channel mix (this scaffold is Meta-only; Bell may add Google/TTD/GA4), the sample campaigns
> (Macbeth / MacKenzie / Season 2026 are illustrative), the `Bell_*` campaign naming filter in
> `sql/01_stg_meta.sql`, and the target/benchmark values in `targets/*.csv` (currency is AUD). The
...
```
 ...
 (Windsor Meta connector,  →  client slice + funnel_stage,  →  reads fact+targets,→  fetches /data.json, rolls
  self-refreshing; shared)     one row per date x ad (fact);    writes fact + flight    up rows[] per the date
        ...
        │                             │                              │                          │
   (no stage-1 loader)         bellshakespeare-export JOB (stage 2)                          bellshakespeare-dash SERVICE (3)
```
...
**A LIGHT treatment matching Bell's brand** (warm off-white canvas + near-black ink + a sage-green
...
- **Creative** — the **top 10 creatives by spend**: real ad headline + body copy + metrics, with the
  ...
  `gs://bidbrain-analytics-bellshakespeare-dash/creatives/<creative_id>` (skips ones already cached), and the dash
...
Login password lives in Secret Manager `bellshakespeare-dash-password` (mounted `DASH_PASSWORD`); agency = **100% Digital**.
...
refresh** (`gs://…-bellshakespeare-dash/reports/…`, keyed by `client + data_through`).

- **One-time standup:** `dash/enable_report_bellshakespeare.ps1` (provisions IAM, mounts the `anthropic-api-key`
...
.\clients\client_bellshakespeare\dash\deploy_dash_bellshakespeare.ps1

# edited a sql/*.sql view → reapply views + re-run the JOB (FORCE_REBUILD bypasses the freshness gate):
.\.venv\Scripts\python.exe clients\client_bellshakespeare\create_views.py
gcloud run jobs execute bellshakespeare-export --region australia-southeast1 --update-env-vars FORCE_REBUILD=1 --wait

# edited job/main.py (the fact / JSON shape) → rebuild + swap + run the JOB:
$IMG = "australia-southeast1-docker.pkg.dev/bidbrain-analytics/bidbrain/bellshakespeare-export:$(git rev-parse --short HEAD)"
gcloud builds submit clients\client_bellshakespeare\job --tag $IMG --region australia-southeast1
gcloud run jobs update  bellshakespeare-export --image $IMG --region australia-southeast1
gcloud run jobs execute bellshakespeare-export --region australia-southeast1 --update-env-vars FORCE_REBUILD=1 --wait
```

## Meta breakdowns — audience (age×gender) + placement (`ingest/meta_breakdown_pull.py`)

The **Audience** and **Placement** charts read `client_bellshakespeare.breakdowns` (view over the ISOLATED table
`raw_windsor.bellshakespeare_meta_breakdown`). This is a **separate, bellshakespeare-only** pull — it does **NOT** touch the
...
.\.venv\Scripts\python.exe clients\client_bellshakespeare\ingest\meta_breakdown_pull.py 2026-05-01 <today> out.ndjson
bq load --replace --source_format=NEWLINE_DELIMITED_JSON raw_windsor.bellshakespeare_meta_breakdown out.ndjson `
...
# then re-run the export job so bellshakespeare.json picks it up (FORCE_REBUILD as above)
```
The `bellshakespeare-export` job tolerates the table's absence (`breakdowns` → `[]`), so the dashboard never breaks
...
it always reads whatever `bellshakespeare.json` is currently in the bucket.

## Freshness

`bellshakespeare-export` is **self-gating** on a Cloud Scheduler `*/10` UTC tick (`scheduler.ps1`): each tick
...
| Views | `client_bellshakespeare.{stg_meta, fact, targets, budget}` (+ `seed_targets` / `seed_budget` tables) |
| Job / Service | `bellshakespeare-export` / `bellshakespeare-dash` |
| Data bucket / file | `bidbrain-analytics-bellshakespeare-dash` / `bellshakespeare.json` (report cache in `reports/`) |
| Dash runtime SA | `bellshakespeare-dash-web@bidbrain-analytics.iam.gserviceaccount.com` |
