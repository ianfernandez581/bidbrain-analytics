-- 03_targets: Bell Shakespeare flight targets + pacing. Source of truth is the VERSION-CONTROLLED
-- committed CSV targets/targets.csv, loaded to client_bellshakespeare.seed_targets by seed_static.py.
...
CREATE OR REPLACE VIEW `bidbrain-analytics.client_bellshakespeare.targets` AS
SELECT
  key,
  value,
  status
FROM `bidbrain-analytics.client_bellshakespeare.seed_targets`