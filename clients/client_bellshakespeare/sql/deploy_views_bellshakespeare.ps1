# deploy_views_bellshakespeare.ps1 - reapply the bellshakespeare SQL views then re-run the export JOB after editing
# sql/*.sql. Applies every sql/*.sql via create_views.py (the source-of-truth applier - NEVER edit
# views in the BigQuery console or they drift), then runs bellshakespeare-export so bellshakespeare.json reflects
...
#       .\clients\client_bellshakespeare\sql\deploy_views_bellshakespeare.ps1
...
$REPO_ROOT = Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent  # sql -> client_bellshakespeare -> clients -> repo root
...
Write-Host "Re-running $JOB so bellshakespeare.json reflects the new views ..."
