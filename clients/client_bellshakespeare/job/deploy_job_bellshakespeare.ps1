# deploy_job_bellshakespeare.ps1 - rebuild + redeploy + run ONLY the bellshakespeare export JOB after editing
# job/main.py (the JSON shape the dashboard reads) or job/requirements.txt. Rebuilds the job image,
# deploys it to the bellshakespeare-export Cloud Run job, then runs it once so a fresh bellshakespeare.json lands
# in the bucket. Does NOT touch the dash service, SQL views, or IAM.
#
# Use this when you changed which fields the job emits / how it assembles bellshakespeare.json (e.g. the
# creative-image cache). If you ALSO edited a sql/*.sql view that feeds it, run
# deploy_views_bellshakespeare.ps1 first (it reapplies the views) - then this rebuild re-reads them.
#
#   HOW TO RUN (from anywhere - paths resolve from the script's own folder):
#       .\clients\client_bellshakespeare\job\deploy_job_bellshakespeare.ps1
...
Write-Host "Running $JOB (writes a fresh bellshakespeare.json + caches creative images to the bucket) ..."
