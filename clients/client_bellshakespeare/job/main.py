"""Bell Shakespeare export job (stage 2) — Bell Shakespeare Meta paid media.
...
Reads BigQuery views client_bellshakespeare.{fact, targets, budget}. The raw layer is raw_windsor.perf_meta
...
CLIENT  = "bellshakespeare"
DATASET     = f"client_{CLIENT}"                    # client_bellshakespeare
BUCKET      = f"bidbrain-analytics-{CLIENT}-dash"   # bidbrain-analytics-bellshakespeare-dash
DATA_OBJECT = f"{CLIENT}.json"                      # bellshakespeare.json
...
def build_env(bq, observed):
    ...
    # Isolated Meta breakdown facts (audience age/gender + placement) — bellshakespeare-only table.
