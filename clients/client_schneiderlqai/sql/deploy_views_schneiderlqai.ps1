# deploy_views_schneiderlqai.ps1 - (re)load the seed CSVs then reapply the schneiderlqai SQL views then
...
# drift), THEN runs schneiderlqai-export so schneiderlqai.json reflects the new output. Does NOT rebuild any
# image or redeploy the service.
#
# Needs the repo venv (load_seeds.py + create_views.py use the BigQuery client). Run the one-shot
# deploy_schneiderlqai.ps1 once first if the dataset/job don't exist yet.
#
#   HOW TO RUN (from anywhere - paths resolve from the script's own folder):
#       .\client_schneiderlqai\sql\deploy_views_schneiderlqai.ps1
...
$REPO_ROOT = Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent  # sql -> client_schneiderlqai -> clients -> repo root
...
Write-Host "Re-running $JOB so schneiderlqai.json reflects the new views ..."
