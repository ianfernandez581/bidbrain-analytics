# deploy_schneiderlqai.ps1 - one-shot, idempotent stand-up of the entire client_schneiderlqai pipeline on GCP.
#
...
#
#   HOW TO RUN (from the repo root OR from inside client_schneiderlqai\):
#       .\client_schneiderlqai\deploy_schneiderlqai.ps1
...
$DATASET  = "client_schneiderlqai"
$BUCKET   = "bidbrain-analytics-schneiderlqai-dash"
$JOB      = "schneiderlqai-export"
$SERVICE  = "schneiderlqai-dash"
$JOB_SA   = "schneiderlqai-dash-job@${PROJECT}.iam.gserviceaccount.com"
$WEB_SA   = "schneiderlqai-dash-web@${PROJECT}.iam.gserviceaccount.com"
$PW_SECRET      = "schneiderlqai-dash-password"
$SESSION_SECRET = "schneiderlqai-dash-session-key"
...
# Build context for `gcloud builds submit .` is the repo root. If we're inside client_schneiderlqai\, step up.
if (-not (Test-Path 'clients/client_schneiderlqai/job/cloudbuild.yaml')) {
  if ((Test-Path 'job/cloudbuild.yaml') -and ((Split-Path -Leaf (Get-Location)) -eq 'client_schneiderlqai')) {
    Set-Location ../..; Write-Host "Moved up to repo root: $(Get-Location)"
  } else { Write-Error "Run from the repo root or from inside client_schneiderlqai\."; exit 1 }
}

Write-Host "Deploying client_schneiderlqai to $PROJECT ($REGION)`n"
...
# 5a. Load data/media_plan.csv -> seed_media_plan TABLE (the media-plan targets the job reads).
...
gcloud builds submit clients/client_schneiderlqai/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
gcloud run jobs execute $JOB --region $REGION --project $PROJECT --wait; Must "run export job (write schneiderlqai.json)"
...
if (Test-Path 'clients/client_schneiderlqai/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit clients/client_schneiderlqai/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
  gcloud run deploy $SERVICE --image $WEB_IMG --region $REGION --service-account $WEB_SA `
    --set-env-vars "GCS_BUCKET=${BUCKET},DATA_OBJECT=schneiderlqai.json" `
  ...
  Write-Host "  SKIPPED dash service - client_schneiderlqai\dash\dashboard.html not found."
}
