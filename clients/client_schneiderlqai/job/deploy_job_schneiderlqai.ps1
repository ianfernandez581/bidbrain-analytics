# deploy_job_schneiderlqai.ps1 - rebuild + redeploy + run ONLY the schneiderlqai export JOB after editing
# job/main.py (the JSON shape the dashboard reads) or job/requirements.txt. Rebuilds the job image,
# deploys it to the schneiderlqai-export Cloud Run job, then runs it once so a fresh schneiderlqai.json
# lands in the bucket. Does NOT touch the dash service, SQL views, or IAM.
#
# Use this when you changed which fields the job emits / how it assembles schneiderlqai.json.
# If you ALSO edited a sql/*.sql view that feeds it, run deploy_views_schneiderlqai.ps1 first (it
# reapplies the views) - then this rebuild re-reads the current views.
# For first-time standup use the one-shot deploy_schneiderlqai.ps1.
#
#   HOW TO RUN (from anywhere - paths resolve from the script's own folder):
#       .\client_schneiderlqai\job\deploy_job_schneiderlqai.ps1
#   If you get "running scripts is disabled on this system":
#       Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force

# ---- config (matches deploy_schneiderlqai.ps1 + job/cloudbuild.yaml) ------------
...
Write-Host "Running $JOB (writes a fresh schneiderlqai.json to the bucket) ..."
