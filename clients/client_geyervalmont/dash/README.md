...
**Where this sits:** this client is in **PREVIEW** - there is no `../job/` yet, so `/data.json`
serves the baked-in `placeholder.json` and `dashboard.html` draws the charts from that. Once an
export job exists it will write `geyervalmont.json` to the private bucket, which `/data.json`
prefers automatically. See [`../README.md`](../README.md) -> FLIPPING PREVIEW -> LIVE.
...
- `../gen_placeholder.py` — builds the SAMPLE payload this app serves while in preview.
