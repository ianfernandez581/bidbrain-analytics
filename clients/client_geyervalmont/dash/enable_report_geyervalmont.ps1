# enable_report_geyervalmont.ps1 - ONE-TIME standup for the "Download report" feature (dash/report.py).
...
# geyervalmont-dash service, and bumps the Cloud Run request timeout so the two-stage Claude/Gemini call
...
#       .\clients\client_geyervalmont\dash\enable_report_geyervalmont.ps1 -Key "sk-ant-..." -GeminiKey "AQ...."
#   If you get "running scripts is disabled on this system":
#       Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force
#
# After this runs once, redeploy the dashboard image normally with deploy_dash_geyervalmont.ps1 - the
...
$SA_FALLBACK = "geyervalmont-dash-web@bidbrain-analytics.iam.gserviceaccount.com"   # dash runtime SA (convention)
$SECRET   = "anthropic-api-key"
$GSECRET  = "gemini-api-key"
$BUCKET   = "bidbrain-analytics-geyervalmont-dash"   # private data bucket - report cache lives in reports/
...
else { Write-Host "geyervalmont-dash runtime SA: $SA" }
...
Write-Host "Redeploy the dashboard image with deploy_dash_geyervalmont.ps1 to ship report.py + the new dashboard.html."
