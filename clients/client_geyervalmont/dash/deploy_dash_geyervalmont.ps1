# deploy_dash_geyervalmont.ps1 - redeploy ONLY the geyervalmont dashboard SERVICE after editing
...
# The Geyer Valmont logo ships TWICE on purpose: inlined as a base64 data URI inside dashboard.html
# (a root-relative path does not resolve behind the platform proxy at /d/geyervalmont/) AND as
# dash/logo.png, which the Dockerfile COPYs and main.py serves at /logo.png for the login page and
# the AI deck builder. Re-run gen_placeholder.py, not this script, after editing targets/*.csv.
# placeholder.json ships from this folder (baked into the image).
if (-not (Test-Path (Join-Path $DASH_DIR 'logo.png'))) { Die "no logo.png in $DASH_DIR (login page + deck need it)" }
