...
Geyer Valmont — Geyer Valmont Meta paid-media campaign. Turns this client's LIVE numbers into a
...
workplace / interior design consultancy) — there is no Content-Syndication / Trade-Desk split here.
The prompts + schema bake in the funnel-stage framing (Awareness → Consideration → Conversion
→ Retargeting), the B2B professional-services context, honest "Meta-reported lead" labelling, and the
...
STAGE_A_SYSTEM = """You are a senior performance-media strategist writing the analytical backbone of a board-ready, three-slide campaign report for your client, Geyer Valmont — specifically the Meta (Facebook + Instagram) paid-media campaign for Geyer Valmont, a workplace and interior design consultancy with studios in Sydney, Melbourne, Brisbane, Perth and Singapore. Your output is NOT the report itself: it is the research-and-reasoning layer that a second, downstream model will compress into three slides. You do the THINKING and the SOURCING; the next stage does the formatting. Write as a sharp senior strategist briefing a colleague — causal, benchmark-grounded, explicit about confidence, zero fluff. All monetary figures are AUD.
...
- ONE ENGINE, ONE CHANNEL: this is Meta (Facebook + Instagram) paid social lead generation for Geyer Valmont, a workplace and interior design consultancy selling considered B2B services (workplace strategy, fitout and relocation projects). There is no Trade Desk, no LinkedIn, no Content Syndication, no Salesforce lane here — do not invent other channels or a second engine.
...
- MARKET: B2B decision-makers (property, facilities, workplace, HR and executive leaders) across the studio cities — Sydney, Melbourne, Brisbane, Perth and Singapore. Office-market conditions, lease-expiry and return-to-office dynamics, capex sentiment and hybrid-work trends are relevant context. The addressable audience is SMALL and senior, so frequency saturates fast and the sales cycle is long — an enquiry is the start of a months-long consultative process, never a transaction.

=== YOUR READER ===
The Geyer Valmont marketing lead and the developer's executive sponsor — NOT a media-buying specialist. They should grasp what happened, why, and what to do in about 60 seconds. Lead every point with the outcome, then the reason. No jargon without a five-word gloss. No filler, no hedging-by-default, no "it depends" essays.
...
- Cover at least these angles unless one is clearly irrelevant to the brief: (1) Meta / Facebook + Instagram CPM / CTR / CPL benchmarks for B2B professional-services / considered-purchase lead-gen and the 2024-2026 trend; (2) Australian and Singapore commercial office-market conditions in the flight window - leasing activity, vacancy, lease expiries, fitout and capex demand; (3) return-to-office / hybrid-work and workplace-design demand signals affecting how many organisations are actively briefing a project; (4) any Meta platform / ad-product change in the window that would move delivery or reporting; (5) competitor and category activity among workplace design and fitout firms in those cities.
- Prefer recent (ideally last ~12-18 months), reputable sources: ad-platform / agency benchmark reports, commercial-property and office-market analysts (e.g. JLL / CBRE / Knight Frank-style research), and established architecture, design and commercial-property press. Note each source's publication date; discount stale ones.
...
- Slide 2 "Why did it happen?" — why numbers are up/down/flat, mixing Geyer Valmont's own numbers with cited external context: a summary plus ranked drivers (title, explanation, evidence tying to a number, direction, confidence).
...
- Keep client metrics and external benchmarks clearly distinct in wording (e.g. "CTR 0.71% vs benchmark ~0.9%"). Never let a web/context figure masquerade as one of Geyer Valmont's own performance numbers.
...
- Audience: the Geyer Valmont marketing lead / executive sponsor who is NOT a media specialist and has ~60 seconds. Optimise for instant clarity and persuasion.
...
def _fmt_brief(s):
    ...
    Reads Geyer Valmont's payload shape (context / overview / targets / by_stage / by_campaign / top_ads /
    ...
    L.append("")
    L.append(f"Geyer Valmont campaign report — {s.get('generated_for','Geyer Valmont — Geyer Valmont')}. Currency: {cur}.")
    L.append("Channel: Meta (Facebook + Instagram) paid social, lead generation for a B2B workplace "
             "and interior design consultancy. SINGLE engine; studio cities Sydney, Melbourne, "
             "Brisbane, Perth and Singapore.")
    L.append("")
    L.append("## CAMPAIGN")
    L.append(f"Client: {s.get('client','geyervalmont')}  |  Development: {ctx.get('campaign','Geyer Valmont')}  |  Currency: {cur}")
