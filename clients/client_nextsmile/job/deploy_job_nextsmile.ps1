# deploy_job_nextsmile.ps1 - rebuild + redeploy + run ONLY the nextsmile export JOB after editing
# job/main.py (the JSON shape the dashboard reads) or job/requirements.txt. Rebuilds the job image,
# deploys it to the nextsmile-export Cloud Run job, then runs it once so a fresh nextsmile.json lands
# in the bucket. Does NOT touch the dash service, SQL views, or IAM.
#
# Use this when you changed which fields the job emits / how it assembles nextsmile.json (e.g. the
# creative-image cache). If you ALSO edited a sql/*.sql view that feeds it, run
# deploy_views_nextsmile.ps1 first (it reapplies the views) - then this rebuild re-reads them.
#
#   HOW TO RUN (from anywhere - paths resolve from the script's own folder):
#       .\clients\client_nextsmile\job\deploy_job_nextsmile.ps1
...
Write-Host "Running $JOB (writes a fresh nextsmile.json + caches creative images to the bucket) ..."
