"""Next Smile Australia export job (stage 2) — Next Smile Australia Meta paid media.
...
Reads BigQuery views client_nextsmile.{fact, targets, budget}. The raw layer is raw_windsor.perf_meta
...
CLIENT  = "nextsmile"
DATASET     = f"client_{CLIENT}"                    # client_nextsmile
BUCKET      = f"bidbrain-analytics-{CLIENT}-dash"   # bidbrain-analytics-nextsmile-dash
DATA_OBJECT = f"{CLIENT}.json"                      # nextsmile.json

# Flight identity (the budget seed has the dates; this is the campaign_key to read).
FLIGHT_KEY = "NEXTSMILE"
...
def build_env(bq, observed):
    ...
    # Isolated Meta breakdown facts (audience age/gender + placement) — nextsmile-only table.
