# deploy_views_nextsmile.ps1 - reapply the nextsmile SQL views then re-run the export JOB after editing
# sql/*.sql. Applies every sql/*.sql via create_views.py (the source-of-truth applier - NEVER edit
# views in the BigQuery console or they drift), then runs nextsmile-export so nextsmile.json reflects
...
#       .\clients\client_nextsmile\sql\deploy_views_nextsmile.ps1
...
$REPO_ROOT = Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent  # sql -> client_nextsmile -> clients -> repo root
...
Write-Host "Re-running $JOB so nextsmile.json reflects the new views ..."
