"""Next Smile Australia dashboard web app (Cloud Run service).
...
`nextsmile.json` from GCS at `/data.json`. All presentation logic — the Executive /
...
DATA_OBJECT = os.environ.get("DATA_OBJECT", "nextsmile.json")   # object inside it
...
# PLACEHOLDER data baked into the container — a Next Smile Australia-branded SAMPLE payload (flagged
...
# the real nextsmile.json to the bucket, /data.json serves THAT instead and the banner disappears.
try:
...
# /bb_deck.js direct, or /d/nextsmile/bb_deck.js through the platform proxy).
try:
...
<title>Next Smile Australia Dashboard</title>
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  body{min-height:100vh;display:flex;align-items:center;justify-content:center;
       ...
       background:#F5ECDB;color:#34281C;position:relative;overflow:hidden}
  body::before{content:'';position:absolute;inset:0;pointer-events:none;
       background:radial-gradient(760px 420px at 50% -8%, rgba(43,108,230,.16), transparent 62%),
                  radial-gradient(560px 320px at 50% -2%, rgba(176,137,90,.18), transparent 66%)}
  ...
  .card{position:relative;width:100%;max-width:390px;padding:40px 34px;background:#FFFFFF;
        border:1px solid #E7DBC7;border-radius:12px;
        box-shadow:0 22px 60px rgba(31,45,36,.18)}
  .logo-wrap{text-align:center;margin-bottom:24px}
  .logo-wrap img{max-height:118px;max-width:270px;display:inline-block}
  .loginmark{display:inline-flex;flex-direction:column;align-items:center;line-height:.82;color:#5A4A38}
  .loginmark .l1,.loginmark .l2{font-size:30px;font-weight:800;letter-spacing:2px}
  .loginmark .aus{margin-top:4px;font-size:11px;font-weight:800;letter-spacing:3px;color:#fff;background:#2B6CE6;padding:2px 12px;border-radius:3px}
  .brand{font-size:10px;font-weight:800;letter-spacing:2.2px;color:#8A7A63;margin-bottom:8px;text-transform:uppercase}
  h1{font-size:22px;font-weight:900;margin:0 0 5px;letter-spacing:0}
  p{font-size:13px;color:rgba(52,40,28,.62);margin:0 0 24px}
  input{width:100%;padding:13px 15px;font-size:15px;color:#34281C;background:#FBF5EA;
        border:1px solid #D8C8AC;border-radius:8px;outline:none;transition:border-color .15s}
  input:focus{border-color:#2B6CE6}
  input::placeholder{color:rgba(52,40,28,.42)}
  button{width:100%;margin-top:14px;padding:13px;font-size:15px;font-weight:700;cursor:pointer;
         background:#2B6CE6;color:#fff;border:none;border-radius:8px;
         transition:transform .1s ease,box-shadow .2s ease;letter-spacing:.3px}
  button:hover{transform:translateY(-1px);box-shadow:0 8px 22px rgba(43,108,230,.32)}
...
/* BB-LOGIN-KIT:css v1 */
  ...
  /* ==========================================================================
     ...
     ========================================================================== */
  :root{--bl-accent:rgb(43,108,230);--bl-glow:rgba(43,108,230,0.42);--bl-ease:cubic-bezier(.22,1,.36,1)}
  ...
  .bb-lgfx .o1{width:62vw;height:56vh;top:-16%;left:-10%;animation:blOrb1 24s ease-in-out infinite;
    background:radial-gradient(circle,rgba(43,108,230,0.072) 0%,transparent 68%)}
  .bb-lgfx .o2{width:54vw;height:48vh;bottom:-18%;right:-12%;animation:blOrb2 29s ease-in-out infinite;
    background:radial-gradient(circle,rgba(176,137,90,0.05) 0%,transparent 68%)}
  .bb-lgfx .o3{width:46vw;height:42vh;top:28%;right:4%;animation:blOrb3 33s ease-in-out infinite;
    background:radial-gradient(circle,rgba(91,143,240,0.044) 0%,transparent 70%)}
  ...
  .bb-pw-t:hover{color:var(--bl-accent);border-color:var(--bl-accent);background:rgba(43,108,230,0.1)}
...
<div class="bb-lgfx" aria-hidden="true"><span class="o1"></span><span class="o2"></span><span class="o3"></span></div><!-- /BB-LOGIN-KIT:fx -->
  <form class="card" method="POST" action="/login">
    <div class="logo-wrap">
      <span class="loginmark"><b class="l1">NEXT</b><b class="l2">SMILE</b><span class="aus">AUSTRALIA</span></span>
    </div>
    <div class="brand">BidBrain · Next Smile Australia</div>
...
def data():
    ...
    #
    # PLACEHOLDER FALLBACK: until the export job has written a real nextsmile.json to the bucket (i.e.
...
def report_route():
    ...
    ckey = "".join(c for c in str(summary.get("client") or "nextsmile").lower()
