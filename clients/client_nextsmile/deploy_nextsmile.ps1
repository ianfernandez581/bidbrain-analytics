# deploy_nextsmile.ps1 - one-shot, idempotent stand-up of the client_nextsmile dashboard on GCP.
#
# TWO PHASES (Next Smile Australia has no live data connected yet):
...
#                             Only valid ONCE real Next Smile Australia Meta data flows into raw_windsor.perf_meta
#                             (campaigns prefixed 'Next Smile Australia_') AND raw_windsor.nextsmile_meta_breakdown
#                             exists (see ingest/meta_breakdown_pull.py). Running the job writes the
#                             real nextsmile.json to the bucket, which then AUTOMATICALLY takes over from
#                             the placeholder (main.py /data.json prefers the bucket) and the banner
#                             clears on its own.
#
#   HOW TO RUN (from the repo root OR from inside client_nextsmile\):
#       .\clients\client_nextsmile\deploy_nextsmile.ps1              # placeholder service
#       .\clients\client_nextsmile\deploy_nextsmile.ps1 -WithData    # full pipeline (once data is connected)
...
$REPO     = "bidbrain"
$DATASET  = "client_nextsmile"
$BUCKET   = "bidbrain-analytics-nextsmile-dash"
$JOB      = "nextsmile-export"
$SERVICE  = "nextsmile-dash"
$JOB_SA   = "nextsmile-dash-job@${PROJECT}.iam.gserviceaccount.com"
$WEB_SA   = "nextsmile-dash-web@${PROJECT}.iam.gserviceaccount.com"
$PW_SECRET      = "nextsmile-dash-password"
$SESSION_SECRET = "nextsmile-dash-session-key"
...
# Build context for `gcloud builds submit .` is the repo root. If we're inside client_nextsmile\, step up.
if (-not (Test-Path 'clients/client_nextsmile/dash/Dockerfile')) {
  if ((Test-Path 'dash/Dockerfile') -and ((Split-Path -Leaf (Get-Location)) -eq 'client_nextsmile')) {
    Set-Location ../..; Write-Host "Moved up to repo root: $(Get-Location)"
  } else { Write-Error "Run from the repo root or from inside client_nextsmile\."; exit 1 }
}

$PYTHON = Join-Path (Get-Location) ".venv\Scripts\python.exe"

Write-Host "Deploying client_nextsmile to $PROJECT ($REGION)  [mode: $(if($WithData){'full pipeline'}else{'placeholder service'})]`n"
...
Ensure-Sa $JOB_SA "Next Smile Australia dashboard export job"
Ensure-Sa $WEB_SA "Next Smile Australia dashboard web service"
...
# ---- 5. Dashboard service (serves placeholder.json until real nextsmile.json exists) -----------
Write-Host "[5/6] Dashboard service ..."
$WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
gcloud builds submit clients/client_nextsmile/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
gcloud run deploy $SERVICE --image $WEB_IMG --region $REGION --service-account $WEB_SA `
  --set-env-vars "GCS_BUCKET=${BUCKET},DATA_OBJECT=nextsmile.json" `
...
if ($WithData) {
  ...
  & $PYTHON clients/client_nextsmile/seed_static.py; Must "seed targets/budget"
  & $PYTHON clients/client_nextsmile/create_views.py; Must "apply views (raw_windsor.perf_meta + nextsmile_meta_breakdown must exist)"
  $JOB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${JOB}:${SHA}"
  gcloud builds submit clients/client_nextsmile/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
  ...
  gcloud run jobs execute $JOB --region $REGION --project $PROJECT --update-env-vars FORCE_REBUILD=1 --wait; Must "run export job (write nextsmile.json)"
  ...
  Write-Host "  Real nextsmile.json written -> the dashboard now shows LIVE data (placeholder banner clears)."
} else {
  Write-Host "[6/6] Data pipeline SKIPPED (placeholder mode). Re-run with -WithData once Next Smile Australia data is connected."
}

Write-Host "`n============================================================"
Write-Host "  DONE. Next Smile Australia dashboard is live (password-gated):"
Write-Host "    $URL"
if (-not $WithData) { Write-Host "  Showing SAMPLE data (placeholder banner). Connect data, then re-run with -WithData." }
Write-Host "  Portal tile: run bidbrain-platform\dash\add_nextsmile_placeholder.py --yes (or seed_registry)."
Write-Host "============================================================"
