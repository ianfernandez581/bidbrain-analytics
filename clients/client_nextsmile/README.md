# client_nextsmile — Next Smile Australia (100% Digital) — PLACEHOLDER / ONBOARDING

> **Status (2026-07-04): placeholder.** This is a full, Next Smile Australia-branded dashboard scaffold **cloned
> from `client_geocon`** (the lean Meta paid-media template) for onboarding **Next Smile Australia** as a **100%
...
>
> **Theme:** a **LIGHT** treatment matching Next Smile's brand — warm **sand** canvas (`#F5ECDB`), warm-brown
> ink (`#34281C`), and a **royal-blue accent** (`#2B6CE6`, their CTA colour), with a warm-tan secondary — the
> same recolour-the-`:root` approach. Logo = the **"NEXT / SMILE / AUSTRALIA"** lockup recreated in CSS (blue
> AUSTRALIA tab; swap in the official SVG when available); the 100% Digital light logo sits in the topbar.
> Regenerate the sample data with `.\.venv\Scripts\python.exe clients\client_nextsmile\gen_placeholder.py`.
>
> **TBD at onboarding (this is a high-ticket, geo-fragmented healthcare lead-gen account — see the brief):**
> the scaffold is Meta-shaped (their audience skews 45-70+, Meta-heavy) but Next Smile will need **per-clinic
> geo campaigns** (7 Super Clinics + regional partners) and **phone-call tracking** as a first-class
> conversion; the real scoreboard is **cost-per-booked-consult per clinic**, not raw form-fills. Sample
> campaigns (All-on-4 awareness / suitability quiz / consult bookings / super) are illustrative. Mind the
> **AHPRA** constraints on testimonials, health claims and super-release messaging in any real ad copy. The
> `NextSmile_*` campaign filter in `sql/01_stg_meta.sql` + targets in `targets/*.csv` (AUD) are placeholders.
...
```
 ...
 (Windsor Meta connector,  →  client slice + funnel_stage,  →  reads fact+targets,→  fetches /data.json, rolls
  self-refreshing; shared)     one row per date x ad (fact);    writes fact + flight    up rows[] per the date
        ...
        │                             │                              │                          │
   (no stage-1 loader)         nextsmile-export JOB (stage 2)                          nextsmile-dash SERVICE (3)
```
...
**A LIGHT treatment matching Next Smile's brand** (warm off-white canvas + near-black ink + a royal-blue
accent and a soft shadow instead of the dark house glow). One file,
...
- **North-star = qualified leads (MODELLED).** Meta reports RAW enquiries only, so qualified leads =
  ...
  spend=sage, cost=sage, CTR=amber.
...
- **Creative** — the **top 10 creatives by spend**: real ad headline + body copy + metrics, with the
  ...
  `gs://bidbrain-analytics-nextsmile-dash/creatives/<creative_id>` (skips ones already cached), and the dash
...
Login password lives in Secret Manager `nextsmile-dash-password` (mounted `DASH_PASSWORD`); agency = **100% Digital**.
...
refresh** (`gs://…-nextsmile-dash/reports/…`, keyed by `client + data_through`).

- **One-time standup:** `dash/enable_report_nextsmile.ps1` (provisions IAM, mounts the `anthropic-api-key`
...
.\clients\client_nextsmile\dash\deploy_dash_nextsmile.ps1

# edited a sql/*.sql view → reapply views + re-run the JOB (FORCE_REBUILD bypasses the freshness gate):
.\.venv\Scripts\python.exe clients\client_nextsmile\create_views.py
gcloud run jobs execute nextsmile-export --region australia-southeast1 --update-env-vars FORCE_REBUILD=1 --wait

# edited job/main.py (the fact / JSON shape) → rebuild + swap + run the JOB:
$IMG = "australia-southeast1-docker.pkg.dev/bidbrain-analytics/bidbrain/nextsmile-export:$(git rev-parse --short HEAD)"
gcloud builds submit clients\client_nextsmile\job --tag $IMG --region australia-southeast1
gcloud run jobs update  nextsmile-export --image $IMG --region australia-southeast1
gcloud run jobs execute nextsmile-export --region australia-southeast1 --update-env-vars FORCE_REBUILD=1 --wait
```

## Meta breakdowns — audience (age×gender) + placement (`ingest/meta_breakdown_pull.py`)

The **Audience** and **Placement** charts read `client_nextsmile.breakdowns` (view over the ISOLATED table
`raw_windsor.nextsmile_meta_breakdown`). This is a **separate, nextsmile-only** pull — it does **NOT** touch the
...
.\.venv\Scripts\python.exe clients\client_nextsmile\ingest\meta_breakdown_pull.py 2026-05-01 <today> out.ndjson
bq load --replace --source_format=NEWLINE_DELIMITED_JSON raw_windsor.nextsmile_meta_breakdown out.ndjson `
...
# then re-run the export job so nextsmile.json picks it up (FORCE_REBUILD as above)
```
The `nextsmile-export` job tolerates the table's absence (`breakdowns` → `[]`), so the dashboard never breaks
...
it always reads whatever `nextsmile.json` is currently in the bucket.

## Freshness

`nextsmile-export` is **self-gating** on a Cloud Scheduler `*/10` UTC tick (`scheduler.ps1`): each tick
...
| Views | `client_nextsmile.{stg_meta, fact, targets, budget}` (+ `seed_targets` / `seed_budget` tables) |
| Job / Service | `nextsmile-export` / `nextsmile-dash` |
| Data bucket / file | `bidbrain-analytics-nextsmile-dash` / `nextsmile.json` (report cache in `reports/`) |
| Dash runtime SA | `nextsmile-dash-web@bidbrain-analytics.iam.gserviceaccount.com` |
