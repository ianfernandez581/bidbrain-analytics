# deploy_caltex.ps1 - one-shot, idempotent stand-up of the client_caltex dashboard on GCP.
#
# TWO PHASES (Caltex has no live data connected yet):
...
#                             clears on its own.
#
#   HOW TO RUN (from the repo root OR from inside client_caltex\):
#       .\clients\client_caltex\deploy_caltex.ps1              # placeholder service
#       .\clients\client_caltex\deploy_caltex.ps1 -WithData    # full pipeline (once data is connected)
...
$BUCKET   = "bidbrain-analytics-caltex-dash"
$JOB      = "caltex-export"
$SERVICE  = "caltex-dash"
$JOB_SA   = "caltex-dash-job@${PROJECT}.iam.gserviceaccount.com"
$WEB_SA   = "caltex-dash-web@${PROJECT}.iam.gserviceaccount.com"
$PW_SECRET      = "caltex-dash-password"
$SESSION_SECRET = "caltex-dash-session-key"
...
# Build context for `gcloud builds submit .` is the repo root. If we're inside client_caltex\, step up.
if (-not (Test-Path 'clients/client_caltex/dash/Dockerfile')) {
  if ((Test-Path 'dash/Dockerfile') -and ((Split-Path -Leaf (Get-Location)) -eq 'client_caltex')) {
    Set-Location ../..; Write-Host "Moved up to repo root: $(Get-Location)"
  } else { Write-Error "Run from the repo root or from inside client_caltex\."; exit 1 }
}

$PYTHON = Join-Path (Get-Location) ".venv\Scripts\python.exe"

Write-Host "Deploying client_caltex to $PROJECT ($REGION)  [mode: $(if($WithData){'full pipeline'}else{'placeholder service'})]`n"
...
Ensure-Sa $JOB_SA "Caltex dashboard export job"
Ensure-Sa $WEB_SA "Caltex dashboard web service"
...
# ---- 5. Dashboard service (serves placeholder.json until real caltex.json exists) -----------
Write-Host "[5/6] Dashboard service ..."
$WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
gcloud builds submit clients/client_caltex/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
gcloud run deploy $SERVICE --image $WEB_IMG --region $REGION --service-account $WEB_SA `
  --set-env-vars "GCS_BUCKET=${BUCKET},DATA_OBJECT=caltex.json" `
...
if ($WithData) {
  ...
  gcloud run jobs execute $JOB --region $REGION --project $PROJECT --update-env-vars FORCE_REBUILD=1 --wait; Must "run export job (write caltex.json)"
  ...
  Write-Host "  Real caltex.json written -> the dashboard now shows LIVE data (placeholder banner clears)."
} else {
  Write-Host "[6/6] Data pipeline SKIPPED (placeholder mode). Re-run with -WithData once Caltex data is connected."
}

Write-Host "`n============================================================"
Write-Host "  DONE. Caltex dashboard is live (password-gated):"
Write-Host "    $URL"
if (-not $WithData) { Write-Host "  Showing SAMPLE data (placeholder banner). Connect data, then re-run with -WithData." }
Write-Host "  Portal tile: run bidbrain-platform\dash\add_caltex_placeholder.py --yes (or seed_registry)."
Write-Host "============================================================"
