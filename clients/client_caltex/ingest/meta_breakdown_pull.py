"""
Caltex-ONLY Meta breakdown pull — audience (age/gender), placement, region.
...
ISOLATION (by design — this cannot affect any other client):
  ...
  * Writes NDJSON destined for a NEW caltex-only table `raw_windsor.caltex_meta_breakdown`
    (loaded via `bq load --replace` in the deploy step). It does NOT write perf_meta.
  * Read-only Windsor calls, scoped to Caltex's account (3754165911553001) and filtered to
    `Caltex_` campaigns before anything is written.
...
RUN (key comes from Secret Manager via the gcloud CLI, so no ADC needed):
    WINDSOR_API_KEY="$(gcloud secrets versions access latest --secret=windsor-api-key | tr -d '\r\n')" \
      python clients/client_caltex/ingest/meta_breakdown_pull.py 2026-05-01 2026-07-04 out.ndjson
then:
    bq load --replace --source_format=NEWLINE_DELIMITED_JSON \
      raw_windsor.caltex_meta_breakdown out.ndjson \
...
ACCOUNT = "facebook__3754165911553001"          # Caltex's Meta account ("100% Digital - Clients")
...
def main():
    ...
    with open(out, "w", encoding="utf-8") as f:
        for bk, dims in PULLS:
            ...
            kept = 0
            for x in rows:
                if not str(x.get("campaign", "")).startswith("Caltex_"):
                    continue
                f.write(json.dumps({
                ...
                kept += 1
            print(f"  {bk}: {len(rows)} rows fetched, {kept} Caltex rows written")
            total += kept
