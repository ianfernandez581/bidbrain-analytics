# deploy_job_caltex.ps1 - rebuild + redeploy + run ONLY the caltex export JOB after editing
# job/main.py (the JSON shape the dashboard reads) or job/requirements.txt. Rebuilds the job image,
# deploys it to the caltex-export Cloud Run job, then runs it once so a fresh caltex.json lands
# in the bucket. Does NOT touch the dash service, SQL views, or IAM.
#
# Use this when you changed which fields the job emits / how it assembles caltex.json (e.g. the
# creative-image cache). If you ALSO edited a sql/*.sql view that feeds it, run
# deploy_views_caltex.ps1 first (it reapplies the views) - then this rebuild re-reads them.
#
...
Write-Host "Running $JOB (writes a fresh caltex.json + caches creative images to the bucket) ..."
