...
- THE OUTCOME = attention and engagement, honestly framed. The ONLY signals available are DELIVERY AND ENGAGEMENT: impressions vs the impression target and CPM vs target (awareness lane); CTR and CPC quality (consideration lane); spend vs budget and pace. CRITICAL - WHAT IS NOT MEASURED: there is NO on-site outcome in this data at all. Star Card applications / sign-ups are not measured (the tracking pixel is not installed on the application domain), and site visits are NOT reported in this deck either. So: never report, estimate or imply a visit, conversion, application, sign-up, lead or sale figure; never credit raw clicks as conversions; never imply fuel-sales lift from display delivery. If the reader needs an on-site outcome, the honest answer is that it must come from Caltex's own analytics, and the fix is to complete the pixel install on the application domain.
...
- slide3.actions: 3-5 prioritized actions ordered high->low priority, each {title, rationale, priority, effort, area}. title = a concrete imperative move (e.g. "Shift weight from standard display to attention-optimised on CTR evidence", "Refresh the wearing-out 300x250 creative", "Get conversion pixels firing so on-site outcomes become measurable"), never "optimize the campaign". rationale = why, tied to a specific number or a slide-2 driver and the lever it moves. priority in {high, medium, low} (low is valid for monitor/hold-course items). effort in {low, medium, high}. area in {creative, audience, budget_pacing, supply, funnel, measurement} ("measurement" = pixels / reporting / instrumentation). Make them genuinely decision-useful — reallocation, pacing, creative refresh, supply tightening, pixel instrumentation — something a marketer could green-light on Monday.
...
def _fmt_brief(s):
    ...
    L.append("MEASUREMENT SCOPE: nothing after the click is measured. The tracking pixel is not installed "
             "on the Star Card application domain, and site-visit reporting was removed from this account's "
             "dashboard, so NO on-site outcome figure exists - never report or estimate a visit, conversion, "
             "application, sign-up or lead count, rate or cost. The honest read is engagement quality "
             "(CTR / CPC) plus the recommendation to complete the pixel install.")
