...
consideration brand campaign for Caltex fuel retail across QLD+WA, bought via several tactics —
...
Consideration) and the honesty / anti-injection / no-PII guardrails.

MEASUREMENT SCOPE (changed 2026-08-18): this deck reports DELIVERY AND ENGAGEMENT ONLY —
impressions, CPM, clicks, CTR, CPC, spend vs pace. Every site-visit / conversion-slot metric was
removed from the dashboard at the client's request, so `buildReportPayload()` no longer sends
`site_actions` / `post_view_actions` / `post_click_actions` / `cost_per_action` / per-row `actions`,
and the prompts below must never ask for or imply an on-site outcome (visits, applications,
sign-ups, leads, sales). The dashboard and this file are one deliverable: if visits are ever
reinstated there, re-add them here in the same change.
...
REPORT_SCHEMA = _obj({
    ...
    "slide1": _obj({
        ...
        "kpis": {"type": "array", "items": _obj({
            ...
            # "actions" was dropped from this enum 2026-08-18 with the site-visit metrics - there is
            # no on-site outcome in the payload, so an actions KPI could only be invented.
            "area": {"type": "string", "enum": ["reach", "engagement", "efficiency", "budget", "overall"]},
...
- THE FUNNEL (organise your reasoning by stage): Awareness (broad-reach standard display: cheap, wide exposure judged on CPM and impression volume) → Consideration (AI-contextual placements + attention-optimised buying: engaged attention judged on CTR and CPC). The tactic mix across these two lanes is itself a lever.
- THE OUTCOME = attention and engagement, honestly framed. The ONLY signals available are DELIVERY AND ENGAGEMENT: impressions vs the impression target and CPM vs target (awareness lane); CTR and CPC quality (consideration lane); spend vs budget and pace. CRITICAL - WHAT IS NOT MEASURED: there is NO on-site outcome in this data at all. Star Card applications / sign-ups are not reported (the tracking tag is now live on the application site, but The Trade Desk is not yet attributing any sign-up to this campaign), and site visits are NOT reported in this deck either. So: never report, estimate or imply a visit, conversion, application, sign-up, lead or sale figure; never credit raw clicks as conversions; never imply fuel-sales lift from display delivery. If the reader needs an on-site outcome, the honest answer is that it must come from Caltex's own analytics. NEVER recommend installing the pixel on the application domain - that was completed on 2026-09-14. The outstanding measurement steps are attaching the application tag to this campaign's conversion reporting in The Trade Desk, and resolving why it resolves no matchable user IDs.
...
1. HEADLINE — one sentence: the single most important takeaway across all three slides, in plain exec language, leading with the outcome (impressions vs target, cost of attention vs target, budget pace, and the engagement signal), honestly framed.
2. WHAT HAPPENED — a tight read of the numbers since launch. Lead with delivery vs the plan (impressions vs the impression target; CPM vs target; spend vs budget and vs the expected pace), then engagement quality (clicks, CTR vs target, CPC; video completion where video ran), then where it is concentrated by stage and tactic. Quote the brief's figures verbatim. Note the flight window and how much has elapsed. Call out the 3-6 movements a board should see; ignore noise.
...
4. BE HONEST ABOUT THE OUTCOME. The measured outcomes are DELIVERY AND ENGAGEMENT ONLY - impressions, CPM, clicks, CTR, CPC, spend. No on-site outcome is measured: not applications, sign-ups, leads, sales, store visits, and not site visits either. Say so plainly rather than estimating one. Never credit clicks as conversions, and never claim fuel-sales lift from delivery data.
5. FLAG LOW CONFIDENCE EXPLICITLY where data is thin (few days elapsed, pending targets, a single tactic distorting the total, small sample, no outcome measurement beyond the click) or the cause is genuinely uncertain. A well-flagged "we are not sure why" is more useful than a confident guess; thin data is a hypothesis to monitor, not a conclusion.
...
1. NUMERIC BRIEF — the authoritative Caltex figures (context / delivery / engagement / by-stage / by-tactic / top creatives / wear-out / targets). Ground truth.
...
- headline: ONE line a busy executive could read alone and know the campaign's state. Lead with the outcome — impressions delivered vs target and the cost of attention (CPM) vs target, plus budget pace and the engagement signal. Plain language, no jargon, <= ~140 chars.
- overall_status: one-word campaign-health read, driven PRIMARILY by delivery vs the impression target and CPM vs target alongside budget pace, with the engagement signal (CTR/CPC) as the tiebreaker. Use "mixed" when the awareness lane (reach/CPM) and the consideration lane (CTR/CPC) disagree, or "neutral" when data is too thin to call.
- slide1.summary: 1-2 sentences, plain language, leading with delivery vs plan (impressions, CPM, pace), then engagement quality (clicks, CTR, CPC).
- slide1.kpis: 4-6 highlight items ranked most->least important, each {label, value, detail, status, area}. value = the headline figure VERBATIM from the brief (e.g. "1.2M impressions", "A$4.43 CPM", "A$5,497 spend", "0.12% CTR", "A$14.20 CPC"), including units/currency symbol. detail = one crisp clause reading it vs target/benchmark/pace, also from the brief (e.g. "21% of the 6M impression target; CPM A$4.43 vs A$5.00 target"). status in {ahead, on_track, behind, neutral} — a clear-eyed read of THAT metric vs its target/benchmark. area in {reach, engagement, efficiency, budget, overall} — cover the awareness lane (reach/efficiency/budget) AND the consideration lane (engagement); do not let one lane masquerade as the whole story. There is no on-site-outcome area: never emit a visit, conversion or application KPI.
...
- slide3.actions: 3-5 prioritized actions ordered high->low priority, each {title, rationale, priority, effort, area}. title = a concrete imperative move (e.g. "Shift weight from standard display to attention-optimised on CTR evidence", "Refresh the wearing-out 300x250 creative", "Attach the live application tag to the campaign's conversion reporting"), never "optimize the campaign". rationale = why, tied to a specific number or a slide-2 driver and the lever it moves. priority in {high, medium, low} (low is valid for monitor/hold-course items). effort in {low, medium, high}. area in {creative, audience, budget_pacing, supply, funnel, measurement} ("measurement" = pixels / reporting / instrumentation). Make them genuinely decision-useful — reallocation, pacing, creative refresh, supply tightening, pixel instrumentation — something a marketer could green-light on Monday.
- confidence_note: one honest line on the report's overall confidence and its main caveat (short window, thin data, pending targets, no on-site outcome measured because the conversion tag is not installed, source gaps). Empty string if none.
...
- BE HONEST ABOUT THE OUTCOME: the measured outcomes are delivery and engagement ONLY (impressions, CPM, clicks, CTR, CPC, spend). No on-site outcome exists in this data - not site visits, not applications, sign-ups, leads or sales - so never report or imply one. Never credit clicks as conversions, and never describe delivery metrics as business outcomes.
...
def _fmt_brief(s):
    ...
    L.append("Channel: The Trade Desk programmatic display, a mixed awareness + consideration "
             "campaign for the Caltex Star Card fuel card, geo-targeted QLD+WA. SINGLE engine; several "
    ...
    L.append("Outcome scope: DELIVERY AND ENGAGEMENT ONLY (impressions, CPM, clicks, CTR, CPC, spend). "
             "No on-site outcome is measured or reported - no site visits, applications, sign-ups, "
             "leads, sales, store visits or CRM outcomes.")
    L.append("")
    ...
    L.append("")
    L.append("## ENGAGEMENT (the consideration lane)")
    ...
    L.append("MEASUREMENT SCOPE: nothing after the click is reported. The application tracking tag IS "
             "live on the Star Card application site (completed 2026-09-14), but The Trade Desk "
             "attributes no sign-up to this campaign yet, and site-visit reporting was removed from this "
             "account's dashboard, so NO on-site outcome figure exists - never report or estimate a visit, "
             "conversion, application, sign-up or lead count, rate or cost. The honest read is engagement "
             "quality (CTR / CPC). Do NOT recommend installing the pixel - that is done; the open steps "
             "are attaching the tag to this campaign's conversion reporting and resolving its zero "
             "matchable user IDs.")
    ...
    if bg:
        ...
        for r in bg:
            L.append(f"  - {r.get('ad_group')} [{r.get('stage')}; market {r.get('market')}]: "
                     ...
                     f"CPM {_money2(r.get('cpm'))}, CTR {_pct(r.get('ctr'),3)}")
        L.append("")
    ...
    L.append("These figures are authoritative ground truth. Do not alter them; web research is for "
             "explanation/context only. These are DELIVERY AND ENGAGEMENT metrics - never describe them "
             "as applications, sign-ups, leads, sales, site visits or store visits, and never credit "
