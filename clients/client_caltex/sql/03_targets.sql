-- 03_targets: Caltex flight targets + pacing. Source of truth is the VERSION-CONTROLLED
-- committed CSV targets/targets.csv, loaded to client_caltex.seed_targets by seed_static.py.
...
CREATE OR REPLACE VIEW `bidbrain-analytics.client_caltex.targets` AS
SELECT
  key,
  value,
  status
FROM `bidbrain-analytics.client_caltex.seed_targets`