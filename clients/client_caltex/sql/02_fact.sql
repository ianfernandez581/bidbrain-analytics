...
-- whole. Same architecture as the Meta template this client was scaffolded from: the job emits THIS
-- fact as a compact `rows[]` array and the dashboard rolls everything up CLIENT-SIDE (KPIs,
-- by-stage / by-tactic / by-creative, the daily trend, the vs-benchmark delta table), filtered by
-- the chosen date range. That is what makes the date-range filter + CSV "export all data" exact
-- and free.
--
...
-- format label -- a TTD creative virtually always serves one format). Names + tactic/market/stage
-- come via ANY_VALUE (constant within a date x creative). All ratio metrics (CTR/CPM/CPC/cost-per-
-- action/video completion) are recomputed CLIENT-SIDE from these summed components, never stored.
CREATE OR REPLACE VIEW `bidbrain-analytics.client_caltex.fact` AS
SELECT
  date,
  campaign_id,
