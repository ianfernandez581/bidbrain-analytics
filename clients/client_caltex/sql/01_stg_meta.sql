-- 01_stg_meta: filter raw_windsor.perf_meta to Caltex campaigns, daily x campaign x adset x ad grain.
...
-- The prefix filter STARTS_WITH('Caltex_') lets future Caltex campaigns (e.g. The Irving) flow
-- in automatically and is immune to the trailing-space quirk in 'Caltex_Traffic_MayJune 2026'.
CREATE OR REPLACE VIEW `bidbrain-analytics.client_caltex.stg_meta` AS
SELECT
  metric_date                                                          AS date,
  campaign_id,
...
WHERE STARTS_WITH(campaign_name, 'Caltex_')