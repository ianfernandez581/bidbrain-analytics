# deploy_views_caltex.ps1 - reapply the caltex SQL views then re-run the export JOB after editing
# sql/*.sql. Applies every sql/*.sql via create_views.py (the source-of-truth applier - NEVER edit
# views in the BigQuery console or they drift), then runs caltex-export so caltex.json reflects
...
#       .\clients\client_caltex\sql\deploy_views_caltex.ps1
...
$JOB       = "caltex-export"
$REPO_ROOT = Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent  # sql -> client_caltex -> clients -> repo root
...
Write-Host "Re-running $JOB so caltex.json reflects the new views ..."
