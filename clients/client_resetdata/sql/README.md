...
| `03_stg_meta.sql` | `stg_meta` | **Meta filter** — `raw_windsor.perf_meta`, `account_name = 'Reset backup – Ad account'` (**EN-DASH** `–`). `cost` already AUD. **`signup_button_conversions` (the "Signup Button" custom pixel) → `conversions`** — the deliberate lead tracker (~51); the generic `actions_lead` (~2) kept as `platform_leads_actions` for reference. Builds `creative_name` for the creative-mix view. |
