...
-- IN_PROGRESS / ATTEMPTED_TO_CONTACT / …). Grain: (lead_status, owner_name) with an
-- "unassigned" split. (The big status-less pool is mostly a bulk import — surfaced as
-- crm_kpi.queue_unassigned, not exploded here.) Frontend leads on NEW + unassigned.
...
GROUP BY c.lead_status, owner_name, unassigned, status_rank
