...
--
-- B2B "ads → traffic / leads": GA4 website outcomes next to Google + Meta + TTD delivery.
-- Window = 2025-12-01 → latest GA4 day (the live data starts mid-Dec 2025). FX_USD_AUD (1.50)
-- converts TTD's USD spend into the AUD reporting currency (Google + Meta already AUD), so a
-- single combined media-spend figure is valid. ad_* = Google + Meta + TTD combined.
...
SELECT
  ...
  (IFNULL(ga.imps,0)   + IFNULL(me.imps,0)   + IFNULL(td.imps,0))    AS ad_imps,
  (IFNULL(ga.clicks,0) + IFNULL(me.clicks,0) + IFNULL(td.clicks,0))  AS ad_clicks,
  (IFNULL(ga.spend_aud,0) + IFNULL(me.spend_aud,0) + IFNULL(td.spend_aud,0)) AS ad_spend_aud,
  -- combined platform-reported conversions (Google + Meta; TTD reports none)
  (IFNULL(ga.conversions,0) + IFNULL(me.conversions,0))             AS ad_conv
FROM g, ga, me, td;
