...
if (-not (Test-Path 'clients/client_resetdata/job/cloudbuild.yaml')) {
...
& $PYTHON clients/client_resetdata/create_views.py; Must "apply views via create_views.py"
$JOB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${JOB}:${SHA}"
gcloud builds submit clients/client_resetdata/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
if (Test-Path 'clients/client_resetdata/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit clients/client_resetdata/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
