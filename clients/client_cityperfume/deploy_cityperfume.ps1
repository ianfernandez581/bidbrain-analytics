...
if (-not (Test-Path 'clients/client_cityperfume/job/cloudbuild.yaml')) {
...
& $PYTHON "clients\client_cityperfume\create_views.py"; Must "apply views (create_views.py)"
$JOB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${JOB}:${SHA}"
gcloud builds submit clients/client_cityperfume/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
if (Test-Path 'clients/client_cityperfume/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit clients/client_cityperfume/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
