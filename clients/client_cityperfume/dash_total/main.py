"""City Perfume ALL-SALES marketing dashboard web app (Cloud Run service).

This is the 2nd City Perfume dashboard: the all-sales variant (In-store POS + Website +
Marketplace). It is serving-identical to client_cityperfume/dash/main.py and reads the SAME
private cityperfume.json (the export job already ships every channel) — only the bundled
dashboard.html differs (all-sales framing + blended-MER ROAS). Deployed as service
cityperfume-total-dash, reusing the cityperfume-dash-web@ SA, bucket, password and session secrets.
...
<title>City Perfume · All-Sales Dashboard</title>
...
<body>
  <form class="card" method="POST" action="/login">
    ...
    <div class="brand">100% DIGITAL · ALL-SALES DASHBOARD</div>
