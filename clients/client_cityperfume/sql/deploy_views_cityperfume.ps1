# deploy_views_cityperfume.ps1 - reapply the cityperfume SQL views then re-run the export JOB after
# editing sql/*.sql. Applies every sql/*.sql via create_views.py (the source-of-truth applier -
# NEVER edit views in the BigQuery console or they drift), then runs cityperfume-export so
# cityperfume.json reflects the new view output. Does NOT rebuild any image or redeploy the service.
#
# Needs the repo venv (create_views.py uses the BigQuery client). Run the one-shot
# deploy_cityperfume.ps1 once first if the job doesn't exist yet.
#
...
# ---- config (matches deploy_cityperfume.ps1) --------------------------------
$PROJECT   = "bidbrain-analytics"
$REGION    = "australia-southeast1"
$JOB       = "cityperfume-export"
$REPO_ROOT = Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent  # sql -> client_cityperfume -> clients -> repo root
...
Write-Host "Re-running $JOB so cityperfume.json reflects the new views ..."
