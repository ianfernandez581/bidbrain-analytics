...
-- views (23 / 24) would have made three copies of the same precedence logic that must always agree —
-- the repo's "N copies must move together" failure mode. A Search detail table that disagreed with
-- the platform table above it is exactly the defect this prevents. 20 / 23 / 24 all read this view;
...
-- unscoped rows are what 24_search_scope_audit needs to prove nothing fell through.
--
