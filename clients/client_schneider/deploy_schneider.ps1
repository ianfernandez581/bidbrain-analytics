...
if (-not (Test-Path 'clients/client_schneider/job/cloudbuild.yaml')) {
...
gcloud builds submit clients/client_schneider/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
if (Test-Path 'clients/client_schneider/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit clients/client_schneider/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
