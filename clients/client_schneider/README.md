...
`sql/23_search_scope_audit` re-checks all of this **every job run** and the export WARNs on `UNMAPPED` or
...
`sql/04_stg_ad_delivery` → `sql/20_pm_delivery` (unchanged behaviour) and `sql/22_search_campaigns`
(campaign grain, for the brand split) → `job/main.py` (`search_campaigns` + `search_fx`) →
`renderSearch()` / `renderSearchPace()` in `dash/dashboard.html`. Plus `sql/23_search_scope_audit`.

**`sql/04b_campaign_program` is new and shared:** the campaign->program first-match-wins ladder used to be
inlined in `sql/20`, and views 22 and 23 would have made three copies of a precedence rule that must always
...
| Which campaigns a Search brief claims | `data/campaign_map.csv` `match_pattern` → `load_seeds.py` → `sql/04b_campaign_program`. Simulate first, and check `sql/23_search_scope_audit` afterwards | 2 |
...
| BigQuery dataset | `client_schneider` (34 views + 7 CSV-loaded `seed_*` tables) |
...
- [`sql/`](sql/README.md) — the 34 BigQuery views (filter + CS leads + paid delivery + `cs_audience` + the Google Search lane `03b`/`22`/`23` + the shared `04b_campaign_program` tagging + the GA4 Website layer `40-47`, shipped disabled).
