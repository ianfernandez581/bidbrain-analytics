...
| File | What it is |
|---|---|
| `main.py` | The job. `CLIENT = "schneider"` → dataset / bucket / object all follow. Builds the **3-tab mongodb-clone** contract scoped to the 5 lead-gen programs (`CS_PROGRAMS`): reads `cs_by_programme` + `cs_weekly` + `pm_delivery` + `search_campaigns` + `cs_audience` + `seed_media_plan` / `seed_plan_budget` / `seed_campaign_map` (display names) → emits `campaigns[]` (per program: target = Σ MQL+HQL `lead_target`, `cpl_tiers` = spend ÷ lead_target per lead line, `committed_spend`, flight from `seed_plan_budget`), plus `cs_by_programme`, `cs_weekly`, `pm_delivery`, **`search_campaigns` + `search_fx`** (Google Search at campaign × brand-vs-non-brand grain — **no conversion field of any kind**, see `sql/03b`), `cs_audience`, `all_markets`, `window`. It also prints the **Google Search scope audit** (`search_scope_audit`) every run and WARNs on an `UNMAPPED` or `IN_SCOPE_NON_ANZ` campaign — both failure modes are otherwise silent. No GA4 branch (the clone has no website tab). Self-gates on upstream freshness before any rebuild (see below). |
...
`raw_snowflake.tradedesk_apac_all`, `raw_snowflake.google_ads_apac` (Google Search, added 2026-09-02 —
without it a Search-only update would never trigger a rebuild) and `raw_snowflake.salesforce_cs_apac_all`
(so fresh Salesforce leads also trigger one) — and compares against the watermark sidecar
