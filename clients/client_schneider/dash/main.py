"""Schneider Electric (Pacific) dashboard web app (Cloud Run service).

Thin password gate + static server. It renders a login screen, and once a session is
authenticated it serves `dashboard.html` and proxies the private `schneider.json` from
GCS at `/data.json`. All presentation logic — the Portfolio / Spend & Pacing / Delivery
& Funnel / Channel Comparison / Geography tabs and every chart — lives in `dashboard.html`;
...
DATA_OBJECT = os.environ.get("DATA_OBJECT", "schneider.json")    # object inside it
...
<title>Schneider Electric · Pacific Dashboard</title>
<style>
...
/* BB-LOGIN-KIT:css v1 */
  ...
  .bb-lgfx .o3{width:46vw;height:42vh;top:28%;right:4%;animation:blOrb3 33s ease-in-out infinite;
    background:radial-gradient(circle,rgba(196,144,46,0.11) 0%,transparent 70%)}
...
<div class="bb-lgfx" aria-hidden="true"><span class="o1"></span><span class="o2"></span><span class="o3"></span></div><!-- /BB-LOGIN-KIT:fx -->
  <form class="card" method="POST" action="/login">
    ...
    <div class="brand">TRANSMISSION · SCHNEIDER ELECTRIC PACIFIC</div>
...
def report_route():
    ...
    ckey = "".join(c for c in str(summary.get("client") or "schneider").lower()
