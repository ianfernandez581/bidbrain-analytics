...
CONFIG = {
    ...
    "business_model": (
        ...
        "  COST PER LEAD-FORM LEAD IS WITHHELD AT THE CLIENT'S REQUEST (2026-09-08) and no longer "
        "appears anywhere on the dashboard. paid.totals no longer carries cost_per_lead_form_lead - "
        "but it still carries spend and lead_form_leads, because both are headline figures in their "
        "own right, so you could derive the cost yourself. Do not. Report lead_form_leads and "
        "lead_form_opens as VOLUME only, and never state or imply a cost per lead, per form "
        "submission or per enquiry for paid media - not in a KPI, a headline, an insight or a "
        "recommendation. This is a reporting instruction from the client, NOT a data-quality "
        "caveat, so do not explain its absence on a slide. The media-plan cost-per-lead in cs.* is "
        "a SEPARATE, plan-side figure for content syndication and is unaffected - keep using it.\n"
