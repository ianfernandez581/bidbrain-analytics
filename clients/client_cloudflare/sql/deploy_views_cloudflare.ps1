# deploy_views_cloudflare.ps1 - reapply the cloudflare SQL views then re-run the export JOB after
# editing sql/*.sql. Applies every sql/*.sql via create_views.py (the source-of-truth applier -
# NEVER edit views in the BigQuery console or they drift), then runs cloudflare-export so
# cloudflare.json reflects the new view output. Does NOT rebuild any image or redeploy the service.
#
# Needs the repo venv (create_views.py uses the BigQuery client). Run the one-shot deploy script
# once first if the dataset/job don't exist yet.
#
#   HOW TO RUN (from anywhere - paths resolve from the script's own folder):
#       .\client_cloudflare\sql\deploy_views_cloudflare.ps1
