...
- **`20_cs_enriched_leads.sql` is the "Weekly Enriched Leads" lane (2026-09-07, client request)** - two views, `cs_enriched_leads` (one row per lead) and `cs_enriched_weekly` (WEEK_START x CAMPAIGN_ID). Scoped to **five CAMPAIGN_IDs by id, never by name** (two of the five share a byte-identical `CAMPAIGN` string). **The whole point of the file is that `ENRICHED_PHONE_NUMBER` has TWO empty sentinels and neither is NULL**: a literal `'-'` (1,427 rows) AND the literal string `'NA'` (132 rows), with zero NULLs. Handling only the dash still overstates enrichment by 36% (495 vs the true 363) and reported one week as 100% enriched. `ENRICHED_PHONE` is therefore NULL unless the value is not a known sentinel AND contains a digit - normalised HERE, once, so the job and dashboard need only `IS NOT NULL`. Weekly grain is `DATE_TRUNC(DAY, ISOWEEK)` - **`DAY`, never `DT_CREATED`** (a single bulk-load instant, the `14_cf1_cs` trap). Carries the **fifth** copy of the Transmission test-lead predicate, which is NOT a no-op here (9 in-scope test leads) - keep all five identical. `ENRICHMENT_RATE` is for the row's own grain only; anything summing campaigns must re-derive it from the two counts. **THEATRE (APJ + EMEA) is resolved by an INNER JOIN against an explicit per-campaign list** - the scope gate and the theatre resolver are one step, so there is no `ELSE` arm to mis-file a lead onto the wrong theatre (the defect `sql/16` had). APJ ids are a literal list (client-stated scope); EMEA ids come from `seed_cs_emea_campaign_ids` - never re-typed. **`cs_enriched_daily` is what the payload ships**, because the tab is date-range driven and a range cutting mid-week cannot be honoured from week buckets (the `18_cs_compare_v2` rule); `cs_enriched_weekly` survives as the job's reconciliation guard, and the two must always agree. `ENRICHED_STATE` / `DASH_COUNT` / `NA_COUNT` exist for the OPEN question of whether `'-'` means "not yet processed" rather than "not enriched" - see the client README. LIVE.
