...
- **KR + RIG are client-defined CS segments (2026-06-19), redefined in `10_salesforce_leads_live.sql`'s `REGION_GRP`** — they are NO LONGER purely geographic, and the BQ region logic now DIVERGES from the reference Snowflake view (`snowflake_v_salesforce_leads_live.sql`, which keeps the old geographic logic for Cloudflare's own legacy R2 export):
  - **KR** = Country `'Korea, Republic of'` **AND** the 6 ORIGINAL El* campaigns only (3 Roverpath + 3 Final Funnel; seed-driven via `seed_kr_campaign_ids`). Korea leads from the Connectivity-Cloud / Modernize campaigns are excluded → land in `OTHER`. (~164 leads.) **Reverted to this rule 2026-07-02** at the client's request (between 2026-06-25 and 2026-07-02 KR was ALL Korea in the 12 campaigns).
