# enable_report_cloudflare.ps1 - ONE-TIME standup for the portal "Download slides" feature (dash/report.py).
# Grants the cloudflare-dash runtime SA secret-read on the shared anthropic-api-key (+ optional gemini-api-key)
# and bucket-write (for the report cache), mounts the keys on the cloudflare-dash service, and bumps the Cloud
...
#       .\clients\client_cloudflare\dash\enable_report_cloudflare.ps1
#   If you get "running scripts is disabled on this system":
#       Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force
#
# After this runs once, redeploy the dashboard image normally with deploy_dash_cloudflare.ps1 - the secret
...
$BUCKET   = "bidbrain-analytics-cloudflare-dash"   # private data bucket - report cache lives in reports/
...
Write-Host "Redeploy the dashboard image with deploy_dash_cloudflare.ps1 to ship report.py + bb_deck.js + the new dashboard.html."
