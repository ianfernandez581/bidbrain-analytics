# deploy_views_sophiie.ps1 - reapply the sophiie SQL views then re-run the export JOB after editing
# sql/*.sql. Applies every sql/*.sql via create_views.py (the source-of-truth applier - NEVER edit
# views in the BigQuery console or they drift), then runs sophiie-export so sophiie.json reflects
...
#       .\clients\client_sophiie\sql\deploy_views_sophiie.ps1
...
$JOB       = "sophiie-export"
$REPO_ROOT = Split-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) -Parent  # sql -> client_sophiie -> clients -> repo root
...
Write-Host "Re-running $JOB so sophiie.json reflects the new views ..."
