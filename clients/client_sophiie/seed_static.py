r"""
client_sophiie/seed_static.py -- load the VERSION-CONTROLLED target CSVs in targets/ into
BigQuery `client_sophiie.seed_*` tables.

Sophiie's targets are a flat key/value model (flight_budget_aud, cpa_target_aud, cpc_target_aud,
ctr_target, cpm_target_aud, impressions_target, signups_target, ...) loaded to seed_targets; the
budget/flight window is loaded to seed_budget. status is load-bearing: HARD = the campaign's own
KPI settings in The Trade Desk, DERIVED = inferred by us from those (the UI must label these, or a
red delta accuses the campaign of missing a KPI nobody agreed to), PENDING = awaiting sign-off. The views sql/03_targets + sql/04_budget SELECT from these seed tables. Edit the
...
Run:  .\.venv\Scripts\python.exe clients\client_sophiie\seed_static.py
"""
import os
