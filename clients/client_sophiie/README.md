# client_sophiie - Sophiie AI (LIVE pipeline, awaiting the Windsor advertiser grant)
...
`SOPHIIE_2026-Q3_TTD_AU_DISPLAY_PROSPECTING` (advertiser `gjcl0pp`), across three prospecting
audience tiers plus a retargeting ad group, Australia only, judged on the platform's own KPI ladder:
...
individual advertisers, and it has held that seat since 2026-08-25. The mirror looked empty only
because the scheduled ingest walks back from *yesterday*, and its 2026-09-04 21:50 UTC run therefore
reached 09-03 - one day before this campaign's first delivery. A forced range run brought it straight
in. If this dashboard ever looks empty again, check `MAX(metric_date)` for the advertiser BEFORE
concluding anything about grants.

The skin is unique in the estate: the **"Chronicle - marble"** treatment over the **three-layer
aurora**, and it is the only ANIMATED dashboard here. Every design note below still applies
unchanged - the channel swap did not touch the skin.
...
| `job/main.py` | The export job. Reads the three views, emits `sophiie.json`, self-gates on `raw_windsor.perf_the_trade_desk` freshness, and **refuses to upload an empty fact** (the caltex pattern) so a premature run cannot blank the placeholder. |
| `job/deploy_job_sophiie.ps1` | Rebuild + deploy + run the `sophiie-export` job. |
| `create_views.py` | Applies `sql/*.sql` in filename order. |
| `seed_static.py` | Loads `targets/*.csv` -> `seed_targets` / `seed_budget`. **Run BEFORE `create_views.py`** (sql/03 + sql/04 are views over those tables). |
| `scheduler.ps1` | Creates/refreshes the `*/10` Cloud Scheduler trigger for `sophiie-export`. |
| `targets/targets.csv`, `targets/budget.csv` | Committed targets - the source of truth for `seed_targets` / `seed_budget`. Every row carries `HARD` / `DERIVED` / `PENDING`, and the UI renders that status. |
| `deploy_sophiie.ps1` | One-shot standup (service + SA + secrets + bucket). |
...
- `renderPlaceholderBanner()` shows the "This is sample data" banner and flips the topbar pill from
  "Live · The Trade Desk" to "Connecting · sample data".
...
The sample is the REAL campaign's shape, generated by `gen_placeholder.py` from the same
`targets/*.csv` the pipeline seeds from: the four real ad groups (three prospecting tiers plus
retargeting), banner formats rather than social placements, and a current mid-flight window so
every pacing card reads "in progress" (the Bell / Next Smile placeholders were seeded with a window
that has since ended, which is how a placeholder starts looking broken). Regenerate it rather than
hand-editing, and re-run it whenever the target CSVs change so the sample can never contradict the
seed.

## Data contract

`sql/*.sql` view column -> `job/main.py` env dict key -> `dashboard.html` `data.*` key, matched
**by name**. Rename a key at one stage and you must rename it at all three. Top-level keys:
`meta`, `flight`, `benchmarks`, `targets`, `rows[]`.

`rows[]` is one row per **date x campaign x ad group x creative**:

| `sql/02_fact` column | `job/main.py` key | Notes |
|---|---|---|
| `date` | `date` | |
| `campaign_id` / `campaign_name` | `campaign_id` / `campaign` | |
| `ad_group_id` / `ad_group_name` | `ad_group_id` / `ad_group` | Named `<AUDIENCE TIER>_<STAGE CODE>`, e.g. `TIER1-CALLHEAVY_AWR`. |
| `tier` | `tier` | Human label parsed from the ad-group name. Cosmetic - every grouping keys on `ad_group_id`. |
| `market` | `market` | Constant `'Australia'` (single-market buy), carried so the shared model keeps the dimension. |
| `creative_id` / `creative_name` / `ad_format` | `creative_id` / `creative` / `ad_format` | |
...
a real value. A `CASE ... ELSE '<real value>'` on a parsed token converts a rename into **silent
misattribution**; `Unclassified` makes it a named job warning and a visible chip instead
(`md/AGENTS.md`, "Campaign names are NOT stable keys").

### A creative runs in SEVERAL ad groups - never name just one

Trade Desk creatives are attached to the advertiser and served by many ad groups; on this campaign
**every creative runs in all four**. So a per-creative rollup cannot carry one `ad_group` / `stage` /
`tier` without asserting something false. `dominant()` in `dashboard.html` resolves the label to
whichever value carried the most of that creative's delivery and returns how many there were, and
`multiLabel()` renders it as `TIER1-CALLHEAVY_AWR +3 more`.

**Resolve it in ONE place.** `creativeAdGroupLabels()` builds the map once from `byCreative(scoped())`
and both the Creative performance table and the wear-out table read it. They used to resolve it
separately - the performance table over the range, the wear-out table per week - and printed
**different ad groups for the same creative on the same screen**.

### Trend grain follows the window

`trendPeriod()` picks day / week / month from the selected span (<=21d -> day), and every Paid Media
trend caption is written by `setPeriodHint()` from the same function, so no chart can be captioned
"Weekly" while drawing days. This flight is 31 days, so a hardcoded week grain drew **two part-week
buckets** for its first fortnight - both lines diving at each end, reading as a collapse in delivery -
and labelled the first bucket by its ISO Monday, **three days before the campaign started**. The
hero trend has always done this via `syncGrainDefault()`; the Paid Media charts did not.
The **wear-out table stays weekly on purpose** - week-over-week CTR decay is what it measures, and it
has its own honest empty state ("needs 2+ weeks at 5,000+ impressions").

### What the first real data changed (2026-09-05)

Four things only real delivery could have surfaced. All are fixed; they are recorded because each is
a class of defect, not a one-off.

- **`TIER2-QUOTELED`, not `TIER2-QUOTED`.** The tier map guessed the spelling and the real ad group
  fell through to the `INITCAP` fallback, rendering as "Tier2 Quoteled". Both spellings are now
  mapped. The fallback is what made this visible rather than silent - do not replace it with a
  real-value `ELSE`.
- **The projection was computed over the elapsed flight, not the delivering window.** The flight
  opens 09-03; delivery began 09-04. Averaging A$337 across 3 elapsed days projected **A$3,487** of a
  A$10,000 flight - a 65% underspend warning about a campaign running at a rate that lands on budget
  (**A$9,786**). Fixed per md/AGENTS.md ("PACE AGAINST THE BUDGET THAT CAN ACTUALLY SPEND"), and the
  card now NAMES its basis on screen. Note the one deliberate difference from geocon's reference
  impl: `run_days` is measured across the REPORTED window (first..last delivery), not
  `today - first_delivery`, because The Trade Desk refuses same-day data - counting to today would
  halve the run rate on every single run of a day-behind feed.
- **The vs-target deltas had no thin-volume guard on the ad-group table.** Retargeting delivered 245
  impressions and 5 clicks on day one and rendered **"+1261% vs CTR target" in confident green**,
  beside Tier 1's +6% off 22,068 impressions. `lowConfidence()` already existed and was applied to
  the creative table only. The number is still shown - what is withheld is the VERDICT COLOUR, since
  the colour is what turns a ratio into a claim.
- **Creative names shared a 23-character prefix.** Every creative is
  `SOPHIIE_2026-Q3_TTD_AU_<tier>_<concept>_<size>`; the name cell ellipsises at ~285px, so six
  creatives in an ad group all rendered as `SOPHIIE_2026-Q3_TTD_AU_TIER1-CALLHEAVY_C...` -
  indistinguishable. `computeCreativePrefix()` strips the LONGEST COMMON PREFIX of the creatives
  currently on screen, trimmed to a token boundary; the full name stays in a `title`. It is computed,
  never hardcoded, so a rename or a second campaign shrinks the prefix instead of hiding something
  real (93% of each name now visible, up from 56%).

Reconciliation at go-live was EXACT at every stage - raw advertiser -> `stg_ttd` -> `fact` ->
`sophiie.json` -> rendered table all agree on 55,348 impressions / 100 clicks / A$337.46, per ad
group as well as in total. Viewability reads 72% (30,451 viewed / 42,368 tracked, a 76.5% sample) -
`viewed/tracked`, never `viewed/impressions`.

## GO-LIVE

Built and deployed already: the BigQuery dataset `client_sophiie` (views `stg_ttd`, `fact`,
`targets`, `budget` + tables `seed_targets`, `seed_budget`), the `sophiie-dash` Cloud Run service,
and the bucket `gs://bidbrain-analytics-sophiie-dash` (empty).

**The one blocker: advertiser `gjcl0pp` is not in `raw_windsor.perf_the_trade_desk`.** Verify with:

    SELECT advertiser_id, advertiser_name, COUNT(*) n, MIN(metric_date), MAX(metric_date)
    FROM `bidbrain-analytics.raw_windsor.perf_the_trade_desk`
    GROUP BY 1,2 ORDER BY n DESC

Remaining steps, in order:

1. **Grant the advertiser** to the Windsor Trade Desk connector at
   https://onboard.windsor.ai?datasource=tradedesk . Note the caltex precedent: a re-grant can issue
   a **NEW seat id** (484 -> 569 there), so a working re-auth can look like a failure until the
   loader is repointed. The loader also walks back from *yesterday* and TTD refuses same-day dates,
   so the first nightly run after a grant can legitimately return nothing; force a range with
   `ingest/windsor_data_pull/tradedesk/tradedesk_loader.py <from> <to>`.
2. **Deploy the export job**: `.\clients\client_sophiie\job\deploy_job_sophiie.ps1`. Safe to run
   before the grant - the empty-fact guard means it exits 0 without uploading, so the placeholder
   stays live.
3. **Create the scheduler**: `.\clients\client_sophiie\scheduler.ps1` (`*/10`). The job self-gates,
   so the FIRST tick after real rows land publishes automatically with no further action.
4. **Confirm the payload** once rows exist: check the job log's slot audit and the `Unclassified`
   warning, then open the dashboard and confirm the sample banner has cleared on its own (no
   dashboard edit, no redeploy).
5. **Set the dashboard password** so the client can log in: either in the super-admin console (which
   reveals + rotates), or grant their Google/Microsoft email in that console's sign-in access panel.
   Do **not** hand out the 100% Digital agency password - it opens every other 100% Digital client.
6. **Flip the tile LAST, and only once real rows are live.** In `bidbrain-platform/dash/config.py`
   set `status: "active"` and the campaign entry to `"active"`, and mirror it in
   `set_sophiie_tile.py` (run with `--yes`) so a future re-seed does not revert it. **Flipping while
   the bucket is empty publishes the SAMPLE payload to the client** - the banner says so, but a
   client-visible dashboard of invented numbers is not a state to ship deliberately. This is the same
   manual gate geocon uses for `seed_property_map.status`.
7. **Monitoring and sync** (already wired in code, verify after step 2): `sophiie` is in `BQ_CLIENTS`
   in `status_dashboard/job/main.py` with five accuracy checks, and `"sophiie-export"` is in
   `_SYNC_EXPORT_JOBS` in `bidbrain-platform/dash/main.py`. **That second one 404s the Run Admin API
   and puts "Sync all dashboards now" into a permanent red failure until the job is deployed**, so
   step 2 must not be skipped. Add to `SLIDES_CLIENTS` and run `dash/enable_report_sophiie.ps1`
   (needs `roles/aiplatform.user`) if the AI deck is wanted - `/report` is dormant until then.

Update this client's row in `md/AGENTS.md` in the same change as any of the above - that table is the
repo's index and a stale row is worse than none.
