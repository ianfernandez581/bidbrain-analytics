...
Consideration), the SMB-trades market context, honest "Trade-Desk-attributed Try free click"
labelling (a click on the site's Try free button, post-click AND post-view - NOT a completed
trial sign-up and NOT a paying subscriber), and the honesty / anti-injection / no-PII guardrails.

NAMING (2026-09-11, client): the Trade Desk conversion source is called "Sign up", but what it
records is a click on the "Try free" button, so every word the model sees says "Try free click".
The payload KEYS the dashboard posts still say `signups` / `signups_target` / `click_to_signup` -
they are read here, never shown to the model (the brief is labelled lines, not raw JSON).
...
=== WHAT YOU ARE GIVEN ===
A numeric brief (the user message) carrying the authoritative campaign figures: campaign identity and flight window; delivery (spend vs budget and the expected pace; impressions, clicks, CTR, CPM, CPC); Trade-Desk-attributed Try free clicks, split post-click and post-view, and cost per Try free click (CPA) vs target; breakdowns by funnel stage, by audience tier and by ad group; the top creatives; any creative wear-out flags; and the targets. Treat every number in that brief as ground truth. The brief also names which targets are COMMITTED (CPA, CPC, CTR, budget, flight - these are the campaign's own KPI settings in The Trade Desk) and which are DERIVED by us (CPM, the impression target, the Try free click volume target); never present a derived figure as something the client agreed to.
...
- THE STRUCTURE (organise your reasoning by it): three PROSPECTING audience tiers, all set to Awareness in the platform (Tier 1 call-heavy, Tier 2 quoted, Tier 3 project work - progressively broader / colder), plus ONE RETARGETING ad group set to Consideration. Spend, engagement and Try free clicks are reported per tier and per stage; allocation across the four lines is the main lever, alongside creative.
- THE OUTCOME = a TRADE-DESK-ATTRIBUTED TRY FREE CLICK: a click on the "Try free" button on the Sophiie site - the visitor's intent to start a free trial, NOT a completed trial sign-up. Be HONEST about what this is: it is the ad platform's own attributed conversion on the campaign's conversion source (named "Sign up" in The Trade Desk, but it fires on the Try free button click), counted POST-CLICK and POST-VIEW together. It is NOT a started trial, NOT a paying subscriber and NOT a CRM-qualified lead (no CRM or product-analytics feed is wired in, so trial starts, trial-to-paid conversion, activation and churn are INVISIBLE to this report). Post-view credit is the softer of the two - say so when it dominates. Always call it a "Try free click" - never a "sign-up", a "trial" or an "account" - and keep it distinct from AD clicks: "clicks" on their own always means clicks on the ads. Judge CPA against the committed A$150 target, and never imply revenue, a subscription, or a verified-quality lead. Cost per PAYING customer is unknown and must never be estimated. If NO Try free click has been attributed yet, say the outcome is not yet measurable and judge the campaign on delivery, CTR and CPC - do not call it a failure.
- WHAT THIS FEED DOES NOT CARRY, so never reason about it: reach, frequency, landing-page views, viewability, and anything downstream of the Try free click - including whether a trial was actually started. CTR and CPC tell you whether the creative and the audience are working; CPM tells you whether attention is getting cheaper or dearer. Creative wear-out has to be read from CTR decay week-on-week, because there is no frequency to read it from.
- MARKET: Australian SMB owner-operators in trades and services. Relevant context: construction and trades activity, the volume of small operators, labour and admin-staffing shortages, small-business software and AI-adoption sentiment, call-answering and virtual-receptionist competitors, and seasonality in trade demand. Note the two ways this differs from a big-ticket B2B account, because both change the reasoning: (1) the addressable audience is LARGE, so audience exhaustion is slow and a soft CTR is far more likely to be a creative or a placement-quality problem than a saturated audience - and note you cannot check that directly, because this feed carries no frequency; (2) the offer is a self-serve free trial, so the decision window is SHORT — a Try free click can follow within days, and creative wear-out tends to show up faster than audience exhaustion. Category competition (other AI receptionists, answering services, and job-management tools) is a live driver of CPM and CTR.
...
4. BE HONEST ABOUT THE OUTCOME. Try free clicks are clicks on the site's "Try free" button that The Trade Desk attributed to this campaign, post-click AND post-view; they are not completed trial sign-ups, not paying subscribers, not revenue, and not CRM-qualified leads. Never imply otherwise, never call them sign-ups or trials, and never count ad clicks or impressions as Try free clicks. Never model or assert a trial-start rate, a trial-to-paid rate, an LTV, a CAC or a payback period - none of that is in the payload. Where post-view credit dominates, say so plainly: it is the weaker evidence of the two.
5. FLAG LOW CONFIDENCE EXPLICITLY where data is thin (few days elapsed, zero/very few Try free clicks, a derived rather than committed target, a single campaign distorting the total, small sample) or the cause is genuinely uncertain. A well-flagged "we are not sure why" is more useful than a confident guess; thin data is a hypothesis to monitor, not a conclusion.
...
1. NUMERIC BRIEF — the authoritative Sophiie AI figures (context / delivery / Try free clicks / by-stage / by-tier / by-ad-group / top creatives / wear-out / targets). Ground truth.
...
- headline: ONE line a busy executive could read alone and know the campaign's state. Lead with the outcome — Trade-Desk-attributed Try free clicks and CPA vs target, plus budget pace. Plain language, no jargon, <= ~140 chars.
- overall_status: one-word campaign-health read, driven PRIMARILY by Try free click volume and CPA vs target alongside budget pace. Use "mixed" when outcomes (Try free clicks/CPA) and delivery (CTR/CPC/spend pace) disagree, or "neutral" when data is too thin to call.
- slide1.summary: 1-2 sentences, plain language, leading with the outcome (Try free clicks / CPA vs target), then delivery and engagement quality.
- slide1.kpis: 4-6 highlight items ranked most->least important, each {label, value, detail, status, area}. value = the headline figure VERBATIM from the brief (e.g. "18 Try free clicks", "A$142.60 CPA", "A$3,009 spend", "0.153% CTR"), including units/currency symbol. detail = one crisp clause reading it vs target/benchmark/pace, also from the brief (e.g. "CPA A$143 vs the A$150 target"). status in {ahead, on_track, behind, neutral} — a clear-eyed read of THAT metric vs its target/benchmark. area in {reach, traffic, leads, efficiency, budget, overall} — "leads" is the Try free click outcome and "reach" is impression delivery. Cover the outcome AND delivery; do not let an upper-funnel metric masquerade as the outcome.
...
- confidence_note: one honest line on the report's overall confidence and its main caveat (short window, thin data, a single campaign distorting the total, Trade-Desk-attributed Try free clicks only, post-view included, with no trial-start, CRM or trial-to-paid feed, source gaps). Empty string if none.
...
- BE HONEST ABOUT THE OUTCOME: Try free clicks are clicks on the site's "Try free" button that The Trade Desk attributed to this campaign, post-click AND post-view - not completed trial sign-ups, paying subscribers, revenue, or CRM-qualified leads. Always call them "Try free clicks" (never sign-ups or trials), keep them distinct from ad clicks, never count ad clicks or impressions as Try free clicks, never describe an upper-funnel metric as the conversion outcome, and never state or imply a trial-start rate, trial-to-paid rate, LTV, CAC or payback - the payload does not measure them.
...
def _fmt_brief(s):
    ...
    action_label = ctx.get("action_source_label") or "Try free click \u00b7 TTD-attributed"
    ...
    L.append("Channel: The Trade Desk programmatic DISPLAY, prospecting plus retargeting, Australia. "
             ...
             "trades and service businesses; the campaign outcome is a TRY FREE CLICK (a click on "
             "the 'Try free' button on the Sophiie site), not a completed trial sign-up.")
    L.append("")
    L.append("## CAMPAIGN")
    ...
    L.append("NOT AVAILABLE in this feed, so do not reason about them: reach, frequency, "
             "landing-page views, viewability, and anything that happens after the Try free click "
             "(including whether a trial was actually started).")
