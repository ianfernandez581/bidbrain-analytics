# deploy_hireright.ps1 - one-shot, idempotent stand-up of the entire client_hireright pipeline on GCP.
#
...
#
#   HOW TO RUN (from the repo root OR from inside client_hireright\):
#       .\client_hireright\deploy_hireright.ps1
...
#
#   DIVERGENCE FROM client_cloudflare/deploy_cloudflare.ps1: HireRight's job is read-only on BigQuery and
#   reads the shared raw layers directly (no Snowflake connection, no src_* landing, no
#   bootstrap-first-failure). So: create dataset -> apply views -> build/deploy job -> run -> dash.
...
$BUCKET   = "bidbrain-analytics-hireright-dash"
$JOB      = "hireright-export"
$SERVICE  = "hireright-dash"
$JOB_SA   = "hireright-dash-job@${PROJECT}.iam.gserviceaccount.com"
$WEB_SA   = "hireright-dash-web@${PROJECT}.iam.gserviceaccount.com"
$PW_SECRET      = "hireright-dash-password"
$SESSION_SECRET = "hireright-dash-session-key"
$SCHEDULE_UTC   = "0 22 * * *"
...
# Build context for `gcloud builds submit .` is the repo root. If we're inside client_hireright\, step up.
if (-not (Test-Path 'client_hireright/job/cloudbuild.yaml')) {
  if ((Test-Path 'job/cloudbuild.yaml') -and ((Split-Path -Leaf (Get-Location)) -eq 'client_hireright')) {
    Set-Location ..; Write-Host "Moved up to repo root: $(Get-Location)"
  } else { Write-Error "Run from the repo root or from inside client_hireright\."; exit 1 }
}

Write-Host "Deploying client_hireright to $PROJECT ($REGION)`n"
...
Ensure-Sa $JOB_SA "HireRight dashboard export job"
Ensure-Sa $WEB_SA "HireRight dashboard web service"
...
# Apply the BigQuery views via the repo venv's create_views.py (BigQuery API), NOT by piping
# each file to `bq query`: Windows PowerShell 5.1 + CP65001 corrupts piped stdin to native
# commands ("Illegal input character"), so the pipe is unreliable. create_views.py reads each
...
Write-Host "     applying views via $venvPy client_hireright/create_views.py"
& $venvPy 'client_hireright/create_views.py'; Must "apply views (create_views.py)"
$JOB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${JOB}:${SHA}"
gcloud builds submit client_hireright/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
gcloud run jobs execute $JOB --region $REGION --project $PROJECT --wait; Must "run export job (write hireright.json)"
...
if (Test-Path 'client_hireright/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit client_hireright/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
  gcloud run deploy $SERVICE --image $WEB_IMG --region $REGION --service-account $WEB_SA `
    --set-env-vars "GCS_BUCKET=${BUCKET},DATA_OBJECT=hireright.json" `
  ...
  Write-Host "  SKIPPED dash service - client_hireright\dash\dashboard.html not found."
}
