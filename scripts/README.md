...
|---|---|---|
| run_platform_local | `.\scripts\run_platform_local.ps1 [-Port 8085]` | Local platform with the knowledge base + its Meetings page and the dashboard assistant's /kb read on: memory backend, DEV=1 (mutations refused), one read-only Secret Manager call for the Gemini key. Super password `local-super`. The Client Assistant stays off. |
...
| `kb_repo_sync.py` | Mirrors **every tracked `.md` in this repo into the knowledge base** at `/kb`, as ~880 SECTION documents under one `Bidbrain analytics` folder. Idempotent: deterministic ids, so a run updates in place, removes what the repo no longer has, and writes nothing when nothing changed. `--dry-run` shows the plan, `--plan-only` needs no credentials at all. Runs itself on every push to main via `.github/workflows/kb-sync.yml`; **edit the markdown in git, never the document in the library** (the next sync replaces it, keeping the old text as a revision). See `bidbrain-platform/README.md` -> "The repo's markdown, mirrored". |
| `setup_kb_sync_auth.ps1` | One-time, idempotent setup for that workflow: the `kb-sync` service account, least-privilege IAM (write scoped to the `kb/` prefix so CI cannot read `platform.json`), and a workload identity pool **pinned to this repo** so no service-account key exists anywhere. |
