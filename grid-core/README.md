...
| **Not launched** | zero delivery (`clientSpend` AND `mediaSpend` both empty/0) **and** the flight has not begun (no `startDate`, or one still in the future). `calc.notLaunched()`, surfaced as `computeRow().notLaunched`. **Excluded from every client rollup and the portfolio totals** (Pulse KPIs, the by-advertiser/by-channel bars, the client summary rows) and reported beside them as a separate **"N not launched"** count — a campaign that has not launched is not underspending, and its budget against zero spend invents a shortfall. Delivery is tested on clientSpend OR mediaSpend, never clientSpend alone (the sync leaves clientSpend null when `spendMult` is unset). A zero-delivery row whose start date has already **passed** stays IN the rollup — late or broken is exactly what pacing should surface. Dates compare at day granularity (a date-only `startDate` is UTC midnight; `new Date()` is a local instant). |
