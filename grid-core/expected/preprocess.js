...
function preprocess(rootDir) {
  ...
  for (const abs of walk(rootDir)) {
    ...
    // Decks and docs reach the model as a filename and nothing else - there is
    // no converter for them here. Flag it on the manifest (not just in the
    // inventory prose) so validate.js can raise it as a real finding: a file
    // that was PRESENT but UNREAD is a silent hole in the audit otherwise.
    if (entry.type === 'deck' || entry.type === 'doc') entry.converted = false;
