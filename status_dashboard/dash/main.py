"""Pipeline Status / Data Health dashboard web app (Cloud Run service).

The "meta dashboard": one gated screen that shows, for every Snowflake-sourced
client, (1) whether the pipeline is stale because Transmission's Snowflake hasn't
...
file only decides *who* may see it, not *what* it shows. The data is assembled by
status_dashboard/job (the status-export Cloud Run job) and written to the bucket.
"""
import os
import hmac
...
DATA_OBJECT = os.environ.get("DATA_OBJECT", "status.json")   # object inside it
...
<style>
  *{box-sizing:border-box}
  ...
  .card{width:100%;max-width:340px;padding:34px 30px;background:#141C2E;
        border:1px solid rgba(255,255,255,.08);border-radius:14px;
        box-shadow:0 12px 44px rgba(0,0,0,.4)}
  .brand{font-size:12px;font-weight:700;letter-spacing:1.4px;color:#34D399;margin-bottom:18px}
  h1{font-size:18px;font-weight:700;margin:0 0 4px}
  p{font-size:13px;color:rgba(255,255,255,.6);margin:0 0 22px}
  input{width:100%;padding:12px 13px;font-size:15px;color:#fff;background:#0B1220;
  ...
  button{width:100%;margin-top:12px;padding:12px;font-size:15px;font-weight:700;cursor:pointer;
         background:#34D399;color:#06231A;border:none;border-radius:9px}
  button:hover{background:#2bbf89}
...
def home():
    ...
    # no-store so a redeploy of the dashboard is picked up immediately, never served
    # stale from the browser or the Cloudflare proxy (matches /data.json).
