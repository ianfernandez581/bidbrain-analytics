# deploy_dash_resetdata.ps1 - redeploy ONLY the resetdata dashboard SERVICE after editing
# dash/dashboard.html or dash/main.py. Rebuilds the dash image and swaps it onto the running Cloud
# Run service; leaves env vars, secrets, the export JOB, SQL views, and all IAM untouched. The fast path.
#
...
# For data-shape changes use job/deploy_job_resetdata.ps1; for SQL view changes use sql/deploy_views_resetdata.ps1.
# For first-time standup (APIs, SAs, IAM, secrets, scheduler) use the one-shot deploy_resetdata.ps1.
#
...
# ---- config (matches deploy_resetdata.ps1 + dash/cloudbuild.yaml) -----------
