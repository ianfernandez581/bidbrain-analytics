# client_resetdata/sql/ — the BigQuery views (stage-2 transform)
...
filter views **01–06** first, then the **07–19** roll-ups that read them). The export job
...
(those are ~0 upstream). Reporting currency is **AUD** throughout. The five sources read straight from
...
| `06_stg_ad_delivery.sql` | `stg_ad_delivery` | **Unified ad-delivery base** — long-format `UNION ALL` of Google + Meta + TTD (platform · campaign · day · creative · imps · clicks · spend_aud · conversions), all spend in AUD. The single source for the campaign-grained roll-ups below. |
| `07_kpi.sql` | `kpi` | One-row headline metrics for the window (`DATE '2025-12-01'` → latest GA4 day). Holds `fx_usd_aud = 1.50`. GA4 outcomes + Google/Meta/TTD delivery; `ad_*` = the three platforms combined; `ad_conv` = Google + Meta (TTD reports none). |
| `08_monthly.sql` | `monthly` | Per-month GA4 (sessions by bucket + mapped paid channels + key events) alongside Google/Meta/TTD delivery — the Overview hero trend (from 2025-12). |
...
The **Platform filter** (dashboard) scopes ad-delivery figures to Google / Meta / TTD, recomputed
client-side from the per-platform columns. The **Campaign filter** is the ad-delivery analogue:
`stg_ad_delivery` (06) folds the three platforms into one long-format fact, and `17–19` roll it up by
