...
Revised same day after approval with two changes, both re-verified: (1) the seed data was
regenerated **unmistakably synthetic** — fictional clients "Example Corp A–F", fictional
staff/contacts, every quote a generic "Sample …" placeholder attributable to no one; same
structural coverage as before (every type/sentiment/source, one submit→feedback→final loop
with a Final link, one zero-feedback report, one client-level incident, context notes,
`meta.sample: true`, identical counts 10/18/3/1) — the full harness re-ran green on the new
data (173/173 across all variants and widths) plus a real-name leak scan over the page and the
print PDF; (2) the ingest now **merges duplicate rows** instead of emitting duplicate cards —
see "Ingest script QA" and the merge-boundaries decision below.
...
2. **Reconciliation — PASS.** Metric cards equal rendered DOM counts at 8 checkpoints (default, all-time, each filter, combined, after clear). At All time the four cards equal the JSON array lengths exactly (10 / 18 / 3 / 1; sentiment 7·6·5). Note: the *default* view is the "Last 3 months" window (Jun–Aug), which correctly excludes the sample May 2026 report (Example Corp D) — cards then read 9/16/2/1, matching what is rendered, per §4.3 "cards describe the currently visible set".
...
13. **Sort — PASS.** Reverse-chronological within sections (Jul→Jun; Example Corp D Jun→May at all-time), not alphabetical.
...
**Ingest script QA** (synthetic CSV, scratchpad only): month forms "June 2026"/"06/2026"/"2026-06"/"Jun 2026"/bare "June" all normalize; "Q3" → `period: null` + flag + renders under Needs review; unknown client "Cloudfare" → excluded + "did you mean 'Cloudflare'?" flag; case-insensitive "mongodb"/"stt" → canonical; multi-line bullet cell → 3 entries with the full raw cell in `context`; final==submitted → null + flag (rule 6); missing-link and blank rows flagged/skipped; legacy `Link to report deck` header accepted; reconciliation counts printed and correct; injected staging page parses identical to its `data.json`. **Merging** (10 dedicated checks, all pass): rows sharing client+campaign+month combine into one report (feedback lines appended, no duplicate card); same submitted-deck URL for the same client also merges even when the campaign was renamed or the month is unparseable; a merged row can fill a missing submitted or final URL on the kept report (incl. after a rule-6 null); every merge is logged in `review_report.txt` with the row it merged into and any field conflict (keep-first policy); the reconciliation block reports "Rows merged into existing reports: n".
...
- Merge boundaries (approved change, 2026-08-08): rows merge on client+campaign+month or on the same submitted-deck URL **for the same client only** — the same URL under two *different* clients is flagged, never merged (merging would silently re-home one client's feedback); two rows whose months are both unparseable are not treated as "the same month" (null ≠ null) and merge only if their URL matches; on any field conflict inside a merge (campaign/month/URL differs) the first row's value is kept and the conflict is written to the review report. Non-canonical clients: excluded from the build but flagged loudly with the nearest canonical name (the UI can only render canonical sections, so "included" would mean silently invisible — worse).
