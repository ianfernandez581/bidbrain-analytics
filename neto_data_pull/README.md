# neto_data_pull/ — mirror Neto / Maropost orders into BigQuery (`raw_neto`)

> A **shared ingest** unit — the third sibling of [`windsor_data_pull/`](../windsor_data_pull/)
> (`raw_windsor`) and [`snowflake_data_pull/`](../snowflake_data_pull/) (`raw_snowflake`). Loads
...
```
Neto API (www.cityperfume.com.au)  ──[orders/orders_loader.py]──►  BigQuery raw_neto.orders  ──►  client views filter their slice (e.g. client_cityperfume/sql/)
```
...
| [`reporting/`](reporting/) | **SAMPLE** reporting views (NOT raw): `v_orders_overview.sql` (one row per order + derived customer history) and `v_sales.sql` (`UNNEST(order_lines)` → product grain). Committed only as worked examples of how to consume `raw_neto.orders`; move them into `client_cityperfume/sql/` when wiring a dashboard. |
| `README.md` | This file. |
