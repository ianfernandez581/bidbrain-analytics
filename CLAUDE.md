...
- Per client `<c>` everything derives from the key: dataset `client_<c>`,
  ...
  (`<c>` ∈ {mongodb, cloudflare, stt, schneider, schneiderlqai, hireright, cityperfume, resetdata, proptrack, tlm, vmch, caltex})
...
**11 LIVE client dashboards (+ `caltex` staged)** — each is `clients/client_<c>/` with `sql/` + `job/` + `dash/`,
dataset `client_<c>`, job `<c>-export`, service `<c>-dash`. All LIVE and self-gating `*/10`
(caltex serves a placeholder until its Windsor TTD data is verified — see its row). The non-derivable facts
...
| `caltex` | **Caltex Star Card** — fuel-card brand campaign: The Trade Desk display, **mixed awareness + consideration**, QLD+WA (TTD advertiser `0lw3hp6`, campaign `85k1vmm` "Caltex Star Card \| QLD+WA \| Jul-Oct 2026") | AUD (no FX) | 4 (+2 seed) | **Deployed 2026-07-29; serves a labelled SAMPLE payload until the first real build.** Rebuilt from the geocon Meta scaffold onto `raw_windsor.perf_the_trade_desk` (NOT Snowflake; shared `windsor-tradedesk-ingest`, agency seat account 484 — the advertiser IS granted, verified against the Windsor API). **First delivery was 2026-07-28**, and the loader walks back from *yesterday*, so its 07-28 21:35 UTC run stopped at 07-27 and the table looked empty — it self-heals on the next nightly run; force it with `tradedesk_loader.py 2026-07-28 <today>`. Tactic + market parsed from ad-group naming `"Tactic \| Market"` (`Display Standard`/`AI Contextual`/`Attention-Optimised` \| `QLD+WA`, all three confirmed live); stage map = Display Standard→Awareness, others→Consideration (ASSUMPTION, one CASE in `sql/01_stg_ttd.sql`). Cost IS populated (AUD, ~A$9.4 CPM on the first 2 days). Site actions = summed anonymous TTD conversion slots (all 12/kind; none firing yet — VERIFY duplicate-pair layout when they do, per the VMCH `{01,03,05}` caveat); `conversion_touch_*` never used. NO reach/frequency in the Windsor TTD feed → creative wear-out = weekly CTR decay. ALL targets `PENDING` (flight 2026-07-28→10-31 + A$30k budget inferred from the campaign name, NOT a signed plan). `job/main.py` REFUSES to upload an empty fact so a premature run can't blank the placeholder. Dashboard = geocon fact-rollup pattern (Overview·Delivery·Creative) + the 2026-07 glow package; spend-mult channel `ttd`. Runbook: `clients/client_caltex/dash/LIVE_URL.md`. |
