...
| `schneider` | **`client_mongodb`-clone Content-Syndication dashboard** (**per-campaign tabs**: Paid Media · Content Syndication · CS Comparison · Other Channels — the tab bar adapts to the selected campaign's channels) — DV360+TTD+LinkedIn paid delivery + Salesforce CS leads, **scoped to 5 lead-gen programs** | AUD (USD@1.50, SGD@1.15) | 27 (+7 seed tables) | **RESTRUCTURED 2026-06-22** from a 6-tab Pacific paid-media dashboard into a **mongodb clone SCOPED to 5 lead-gen programs** {water_env, eba, heavy, global_rebrand, airset} = the 9 SF campaign IDs in `data/salesforce_map.csv`; the other ~20 APAC programs are GONE from the dash (seed tables keep them for the match_pattern tag). Model: **Campaign**=program (**top-nav dropdown**, Cloudflare pattern; the dashboard **defaults to an "All campaigns" portfolio view** — a synthesized pseudo-campaign that aggregates all 5 across Paid Media / Content Syndication / CS Comparison), **Programme**=SF `pillar_label`, **Market**=**Australia / New Zealand only** (region chips; CS leads are AU/NZ-native and `20_pm_delivery` resolves the paid AU/NZ split from **AD_GROUP_NAME** then CAMPAIGN_NAME — the earlier campaign-name-only parse stranded EBA's TTD delivery in an Unmapped/Other bucket; a tiny unsplittable combined-ANZ residual folds into Australia, so there is NO ANZ/Other bucket). Data layer: `sql/17_stg_salesforce` (adds campaign/programme/market; buckets status on **LEAD_STATUS only** — STATUS/LEAD_STATUS_SF are INT64/all-NULL; DAY is already DATE so **NO `DATE()` wrapper**), `18_cs_by_programme`, `19_cs_weekly`, **`20_pm_delivery`** (replicates the old client-side `idOf` match_pattern join IN SQL — first-match-wins across ALL 28 seed rows by `seq`, THEN filter to the 5; market resolved to AU/NZ only). job emits `campaigns[]` (target=Σ MQL+HQL `lead_target`; `cpl_tiers`=spend÷lead_target per media-plan lead line; committed=Σ lead-line spend; flight from `seed_plan_budget`; **+per-campaign `channels[]`+`tabs[]`** — media-plan channels bucketed paid/cs/other by `chan_group`, driving the dynamic per-campaign tab bar; the plan-only **Other Channels** tab = Search/publishers/trade/email, target-only, no warehouse feed), `cs_by_programme`, `cs_weekly`, `pm_delivery`, `all_markets`, `window` — **NO GA4** (40–46 apply but unused). 95 in-flight leads (eba 42/water_env 28/heavy 25; all `New` — CRM-raw, not graded MQLs). **Leads are CLAMPED to each program's flight window** (`stg_salesforce` LEFT JOINs `seed_plan_budget` — `DAY` between flight_start/end), so pre-flight spillover is excluded (EBA had 4 leads before its 2026-05-25 start). heavy+global_rebrand have **NO paid delivery** (leads-only). Seeds CSV-loaded via `load_seeds.py` (run BEFORE `create_views.py`); the media-plan **targets** (`media_plan`/`targets`/`plan_budget`) **plus `campaign_map`** live in the VERSION-CONTROLLED `targets/` dir (the committed-CSV→BQ targets standard, routed by `SRC_DIRS`), the remaining dimension/config seeds in gitignored `data/`. Targets from the media-plan sheet (EBA MQL 157 not 300; W&E/Heavy/EBA budgets changed; NEL added) — flagged in `INTAKE.md`. GATING adds `salesforce_cs_apac_all`. Pacific-carve-out history: `_eda/pacific_eda.md`. **Targets = committed-CSV→BQ (2026-06-25 standard, all 3 target-bearing clients): targets live in a tracked `targets/` dir → `seed_*` table, never a gitignored `data/` CSV.** (schneider's `campaign_map` is now committed in `targets/` too — its `display_name`/`match_pattern` are the BQ source of truth; the remaining non-target seeds — plan_flighting/channel_split/salesforce_map — are still BQ-only.) |
