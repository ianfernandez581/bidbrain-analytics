...
| `caltex` | **STAGED (2026-07-29; placeholder served until Windsor TTD data verified)** — Caltex fuel retail: The Trade Desk display, **mixed awareness + consideration**, QLD+WA (TTD advertiser `0lw3hp6`) | AUD (no FX) | 4 (+2 seed) | Rebuilt from the geocon Meta scaffold onto `raw_windsor.perf_the_trade_desk` (NOT Snowflake; shared `windsor-tradedesk-ingest`, agency seat account 484 — if advertiser `0lw3hp6` isn't in the table, a human must grant it at onboard.windsor.ai + append the account id to `SELECT_ACCOUNTS`). Tactic + market parsed from ad-group naming `"Tactic \| Market"` (`Display Standard`/`AI Contextual`/`Attention-Optimised` \| `QLD+WA`); stage map = Display Standard→Awareness, others→Consideration (ASSUMPTION, one CASE in `sql/01_stg_ttd.sql`). Site actions = summed anonymous TTD conversion slots (all 12/kind; VERIFY duplicate-pair layout once pixels fire — the VMCH `{01,03,05}` caveat); `conversion_touch_*` never used. NO reach/frequency in the Windsor TTD feed → creative wear-out = weekly CTR decay. ALL targets `PENDING` (flight window + A$30k budget are placeholders). Dashboard = geocon fact-rollup pattern (Overview·Delivery·Creative) + the 2026-07 glow package; spend-mult channel `ttd`. Go-live runbook: `clients/client_caltex/dash/LIVE_URL.md`. |
