...
| `cloudflare` | TTD+LinkedIn+Reddit+LINE + CS + 3 single-campaign LinkedIn dashes | USD (LINE JPY→USD@155) | 13 | **On the standard pattern since 2026-06-17** (was the lone Snowflake-modelled exception). BQ now owns the model: staging+model views over `raw_snowflake.*` mirrors + 3 BQ static seed tables (`seed_real_targets`/`seed_tiers`/`seed_line_cf`, pulled once to `data/` — gitignored per `clients/*/data/` — via `pull_static.py`→`seed_static.py`). CS campaign filter (12 IDs) lives in `sql/10_salesforce_leads_live.sql`; CS map is OPPOSITE of mongodb. Pacing tier sub-split is non-deterministic in the source model (742 conflicting tier names, 349 tier-ambiguous accepted leads) — headline lead totals are exact, the Tier 2/3/Other split flickers run-to-run as it always did. The `snowflake_v_*.sql` files are now just reference for Cloudflare's OWN legacy R2 export, NOT our pipeline. |
