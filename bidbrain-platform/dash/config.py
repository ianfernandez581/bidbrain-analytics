...
CLIENTS = {
    ...
    # Onboarding: client_lacevo/ is a built, Lacevo-branded (bone field + a black brand shell, clay
    # accent) PREVIEW dashboard on SAMPLE data. Australian DTC on Shopify - wearable breast pumps,
    # bottle warmer, light-therapy set, spare parts - selling into AU (lacevo.com) and the US
    # (lacevo.us). No feed is connected yet, so like geyervalmont there is deliberately NO sql/ +
    ...
    "lacevo": {
        "name": "Lacevo", "slug": "lacevo", "status": "coming_soon",
        "url": _runapp("lacevo"),   # deployed preview: super-admin-openable, hidden from clients (coming_soon)
        "note": "Dashboard isn't live yet - the structure is ready.",
        "show_pending_row": True,
        "campaigns": [
...
AGENCIES = [
    {
        ...
        "clients": ["cityperfume", "vmch", "tlm", "resetdata", "bellshakespeare", "geocon", "caltex",
                    "nextsmile", "geyervalmont", "sophiie", "lacevo", "raywhiteprojects"],
    },
    {
