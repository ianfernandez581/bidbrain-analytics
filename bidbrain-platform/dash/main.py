...
{% for r in rows %}
  ...
  <div class="card" data-status="{{ st }}" data-client="{{ r.client }}"
       data-agency="{{ agency_of.get(r.client, '') }}"
       data-q="{{ (names.get(r.client, r.client) ~ ' ' ~ (r.text or '') ~ ' ' ~ (r.transcript or '') ~ ' ' ~ (r.ai_summary or '') ~ ' ' ~ (r.reporter or '') ~ ' ' ~ (r.page or '') ~ ' ' ~ st) | lower }}">
...
// An auth or permission refusal must never be reported as "could not save, try again" - retrying is
// the one thing that cannot fix it (the repo-wide rule), so the reason is read off the response.
function fbWhyFailed(resp,fallback){
  if(resp&&(resp.status===401||resp.status===403)){
    return resp.json().catch(function(){return {};}).then(function(j){
      return j.error||'Your session has expired - sign in again.';
    });
  }
