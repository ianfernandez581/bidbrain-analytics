r"""Surgically set the Lacevo tile in the LIVE platform registry - WITHOUT a full re-seed (so it
cannot clobber agency/client edits made through the admin UI).

`seed_registry.py --force` rewrites every agency + client from config.py; this instead does a single
targeted upsert against the live registry JSON in GCS: it attaches the `lacevo` client to the 100%
...
The constants below are the desired STATE. No Lacevo feed is connected, so STATUS is "coming_soon"
with the placeholder NOTE - the tile renders with the greyed COMING SOON chip and the "Dashboard
isn't live yet - the structure is ready." blurb, exactly like Bell Shakespeare, Next Smile and Geyer
Valmont, and a super admin can still open the deployed preview via "Open preview ->".
...
(ian@100.digital) - NOT charles@ (no perms). PowerShell:
    ...
    .\.venv\Scripts\python.exe bidbrain-platform\dash\set_lacevo_tile.py --yes

Without --yes it prints what it WOULD do and the current lacevo state, then exits (dry run).

NOTE - client ACCESS is separate from this tile. The registry keeps no dashboard password for
lacevo yet (`password_hash` is empty). While the tile is coming_soon that is correct: only a super
admin should be opening the preview. When it goes live, either set the Lacevo dashboard password in
the SUPER-ADMIN console (it reveals + rotates) or grant their Google/Microsoft email to this
dashboard in that console's sign-in access panel. Agency-level access (the 100% Digital password)
would expose every other 100% Digital client, so do not hand that out.

NOTE - Lacevo is attached to 100% Digital ONLY. It is deliberately absent from the `extrablack`
agency: that is an EXTERNAL tenant, and dual visibility there is a per-client decision (geocon /
resetdata / geyervalmont), never a default.
"""
import sys
...
URL = "https://lacevo-dash-516554645957.australia-southeast1.run.app/"
NOTE = "Dashboard isn't live yet - the structure is ready."
CAMPAIGN = ("Trading and Paid Media", "/", "coming_soon")
...
def main(write: bool):
    ...
    print(f"live registry: lacevo exists={bool(existing)} | attached to {AGENCY}={attached}")
    if not write:
    ...
    # all for Lacevo, rather than the honest greyed "awaiting connection" one. Drop it to False in
    ...
    print(f"\nDONE. lacevo -> status={c['status']} | campaigns={c.get('campaigns')} "
          f"| in {AGENCY}={KEY in ag.get('client_keys', [])}")
    print("The Lacevo tile now shows on the 100% Digital portal with the preview treatment.")
