# deploy_dash_proptrack.ps1 - redeploy ONLY the proptrack dashboard SERVICE after editing
# dash/dashboard.html or dash/main.py. Rebuilds the dash image and swaps it onto the running Cloud
# Run service; leaves env vars, secrets, the export JOB, SQL views, and all IAM untouched. The common,
# fast path.
#
...
# For data-shape changes use deploy_job_proptrack.ps1; for SQL view changes use deploy_views_proptrack.ps1.
# For first-time standup (APIs, SAs, IAM, secrets, scheduler) use the one-shot deploy_proptrack.ps1.
#
#   HOW TO RUN (from anywhere - paths resolve from the script's own folder):
#       .\client_proptrack\dash\deploy_dash_proptrack.ps1
#   If you get "running scripts is disabled on this system":
#       Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force

# ---- config (matches deploy_proptrack.ps1 + dash/cloudbuild.yaml) -----------
