...
if (-not (Test-Path 'client_proptrack/job/cloudbuild.yaml')) {
...
$sqlFiles = Get-ChildItem 'client_proptrack/sql' -Filter '*.sql' | Sort-Object Name
...
gcloud builds submit client_proptrack/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
if (Test-Path 'client_proptrack/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit client_proptrack/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
