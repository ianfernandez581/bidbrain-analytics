...
#
#   DIVERGENCE FROM client_cloudflare/deploy_cloudflare.ps1: STT's job is read-only on BigQuery and
...
if (-not (Test-Path 'client_STT/job/cloudbuild.yaml')) {
...
$sqlFiles = Get-ChildItem 'client_STT/sql' -Filter '*.sql' | Sort-Object Name
...
gcloud builds submit client_STT/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
if (Test-Path 'client_STT/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit client_STT/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
