# deploy_job_stt.ps1 - rebuild + redeploy + run ONLY the stt export JOB after editing job/main.py
# (the JSON shape the dashboard reads) or job/requirements.txt. Rebuilds the job image, deploys it
# to the stt-export Cloud Run job, then runs it once so a fresh stt.json lands in the bucket.
# Does NOT touch the dash service, SQL views, or IAM.
#
# Use this when you changed which fields the job emits / how it assembles stt.json.
# If you ALSO edited a sql/*.sql view that feeds it, run deploy_views_stt.ps1 first (it reapplies
# the views) - then this rebuild re-reads the current views.
# For first-time standup use the one-shot deploy_stt.ps1.
#
#   HOW TO RUN (from anywhere - paths resolve from the script's own folder):
#       .\client_STT\job\deploy_job_stt.ps1
#   If you get "running scripts is disabled on this system":
#       Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force

# ---- config (matches deploy_stt.ps1 + job/cloudbuild.yaml) ------------------
...
Write-Host "Running $JOB (writes a fresh stt.json to the bucket) ..."
