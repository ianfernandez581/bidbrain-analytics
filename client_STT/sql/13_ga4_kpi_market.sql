-- STT GDC — GA4 headline metrics BY market/property (campaign window) + prior-year baseline.
--
-- Powers the Country filter: the dashboard sums these rows over the selected
-- countries to recompute every GA4 KPI live. One row per market.
CREATE OR REPLACE VIEW `bidbrain-analytics.client_stt.ga4_kpi_market` AS
WITH cur AS (
  SELECT
    market,
