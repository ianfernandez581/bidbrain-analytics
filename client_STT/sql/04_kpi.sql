...
-- The prior-year window gives a rough pre-campaign baseline for the sessions Δ.
CREATE OR REPLACE VIEW `bidbrain-analytics.client_stt.kpi` AS
WITH
g AS (
  SELECT
    MIN(metric_date) AS ga4_start,
    MAX(metric_date) AS ga4_end,
...
SELECT
  ...
  (li.imps + dv.imps)                       AS ad_imps,
  (li.clicks + dv.clicks)                    AS ad_clicks,
  (dv.spend_sgd + li.cost_usd * 1.34)        AS ad_spend_sgd
FROM g, gp, li, dv;
