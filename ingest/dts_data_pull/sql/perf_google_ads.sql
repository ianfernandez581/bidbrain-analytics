...
FROM (
  ...
  FROM `bidbrain-analytics.raw_google_ads.ads_CampaignBasicStats_1054407474` WHERE segments_date >= DATE '2026-08-25'
...
LEFT JOIN (
  ...
  FROM `bidbrain-analytics.raw_google_ads.ads_Campaign_1054407474`
...
LEFT JOIN (
  ...
  FROM `bidbrain-analytics.raw_google_ads.ads_Customer_1054407474`
...
FROM (
  SELECT customer_id, campaign_id, segments_date AS metric_date,
         ...
         CAST(SUM(metrics_conversions) AS NUMERIC)       AS conversions,
         CAST(SUM(metrics_conversions_value) AS NUMERIC) AS conversions_value
  FROM `bidbrain-analytics.raw_google_ads.ads_CampaignBasicStats_1869745895` WHERE segments_date >= DATE '2026-08-25'
  GROUP BY customer_id, campaign_id, segments_date
) s
LEFT JOIN (
  SELECT campaign_id, customer_id, campaign_name, campaign_advertising_channel_type
  FROM `bidbrain-analytics.raw_google_ads.ads_Campaign_1869745895`
  QUALIFY ROW_NUMBER() OVER (PARTITION BY campaign_id ORDER BY _DATA_DATE DESC) = 1
) c USING (campaign_id, customer_id)
LEFT JOIN (
  SELECT customer_id, customer_descriptive_name, customer_currency_code
  FROM `bidbrain-analytics.raw_google_ads.ads_Customer_1869745895`
  QUALIFY ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY _DATA_DATE DESC) = 1
) cust USING (customer_id)
UNION ALL
SELECT
...
FROM (
  SELECT customer_id, campaign_id, segments_date AS metric_date,
         ...
         CAST(SUM(metrics_conversions) AS NUMERIC)       AS conversions,
         CAST(SUM(metrics_conversions_value) AS NUMERIC) AS conversions_value
  FROM `bidbrain-analytics.raw_google_ads.ads_CampaignBasicStats_3451896252` WHERE segments_date <= DATE '2026-08-24'
  GROUP BY customer_id, campaign_id, segments_date
) s
LEFT JOIN (
  SELECT campaign_id, customer_id, campaign_name, campaign_advertising_channel_type
...
FROM (
  SELECT customer_id, campaign_id, segments_date AS metric_date,
         ...
         CAST(SUM(metrics_conversions) AS NUMERIC)       AS conversions,
         CAST(SUM(metrics_conversions_value) AS NUMERIC) AS conversions_value
  FROM `bidbrain-analytics.raw_google_ads.ads_CampaignBasicStats_5457742070` WHERE segments_date >= DATE '2026-08-25'
  GROUP BY customer_id, campaign_id, segments_date
) s
LEFT JOIN (
  SELECT campaign_id, customer_id, campaign_name, campaign_advertising_channel_type
  FROM `bidbrain-analytics.raw_google_ads.ads_Campaign_5457742070`
  QUALIFY ROW_NUMBER() OVER (PARTITION BY campaign_id ORDER BY _DATA_DATE DESC) = 1
) c USING (campaign_id, customer_id)
LEFT JOIN (
  SELECT customer_id, customer_descriptive_name, customer_currency_code
  FROM `bidbrain-analytics.raw_google_ads.ads_Customer_5457742070`
  QUALIFY ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY _DATA_DATE DESC) = 1
) cust USING (customer_id)
    UNION ALL
