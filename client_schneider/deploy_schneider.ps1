# deploy_schneider.ps1 - one-shot, idempotent stand-up of the entire client_schneider pipeline on GCP.
#
...
#
#   HOW TO RUN (from the repo root OR from inside client_schneider\):
#       .\client_schneider\deploy_schneider.ps1
...
$DATASET  = "client_schneider"
$BUCKET   = "bidbrain-analytics-schneider-dash"
$JOB      = "schneider-export"
$SERVICE  = "schneider-dash"
$JOB_SA   = "schneider-dash-job@${PROJECT}.iam.gserviceaccount.com"
$WEB_SA   = "schneider-dash-web@${PROJECT}.iam.gserviceaccount.com"
$PW_SECRET      = "schneider-dash-password"
$SESSION_SECRET = "schneider-dash-session-key"
$SCHEDULE_UTC   = "0 22 * * *"
...
# Build context for `gcloud builds submit .` is the repo root. If we're inside client_schneider\, step up.
if (-not (Test-Path 'client_schneider/job/cloudbuild.yaml')) {
  if ((Test-Path 'job/cloudbuild.yaml') -and ((Split-Path -Leaf (Get-Location)) -eq 'client_schneider')) {
    Set-Location ..; Write-Host "Moved up to repo root: $(Get-Location)"
  } else { Write-Error "Run from the repo root or from inside client_schneider\."; exit 1 }
}

Write-Host "Deploying client_schneider to $PROJECT ($REGION)`n"
...
$sqlFiles = Get-ChildItem 'client_schneider/sql' -Filter '*.sql' | Sort-Object Name
...
gcloud builds submit client_schneider/job --tag $JOB_IMG --region $REGION --project $PROJECT; Must "build export job image"
...
gcloud run jobs execute $JOB --region $REGION --project $PROJECT --wait; Must "run export job (write schneider.json)"
...
if (Test-Path 'client_schneider/dash/dashboard.html') {
  $WEB_IMG = "${REGION}-docker.pkg.dev/${PROJECT}/${REPO}/${SERVICE}:${SHA}"
  gcloud builds submit client_schneider/dash --tag $WEB_IMG --region $REGION --project $PROJECT; Must "build dash image"
  gcloud run deploy $SERVICE --image $WEB_IMG --region $REGION --service-account $WEB_SA `
    --set-env-vars "GCS_BUCKET=${BUCKET},DATA_OBJECT=schneider.json" `
  ...
  Write-Host "  SKIPPED dash service - client_schneider\dash\dashboard.html not found."
}
