# windsor_data_pull/meta/ — Meta / Facebook Ads loader (`raw_windsor.perf_meta`)

> Loads Meta/Facebook Ads performance from Windsor.ai into BigQuery, one row per
> **(ad × date)**. Part of [`windsor_data_pull/`](../README.md) — read that first for the
...
| File | What it does |
|---|---|
| [`create_meta_table.py`](create_meta_table.py) | **One-time.** Creates `raw_windsor.perf_meta` with the full typed schema (~100 columns), partitioned by `metric_date`, clustered by `campaign_id, ad_id`. Idempotent — but it **creates, doesn't alter**: to add columns to an existing table, drop it first (it's safe when empty). |
